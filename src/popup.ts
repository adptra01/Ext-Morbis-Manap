import type { ExtensionConfig, CustomUrl, MessagePayload, PopupDOM, Role } from './types.js';
...
const DOM: PopupDOM = {
  ...
  roleBanner: document.getElementById('roleBanner') as HTMLElement,
};
...
async function bgWrite(msg: MessagePayload): Promise<{ success: boolean } | null> {
  const ok = await bgMessage(msg);
  if (ok) return ok as { success: boolean };
  const c = currentConfig || ({} as ExtensionConfig);
  switch (msg.type) {
    case 'TOGGLE_EXTENSION':
      c.extensionEnabled = msg.enabled as boolean;
      await chrome.storage.sync.set({ extensionConfig: c });
      break;
    case 'TOGGLE_FEATURE':
      if (c.features?.[msg.key as string])
        c.features[msg.key as string].enabled = msg.enabled as boolean;
      await chrome.storage.sync.set({ extensionConfig: c });
      break;
    case 'SET_ROLE':
      c.currentRole = msg.role as Role;
      await chrome.storage.sync.set({ extensionConfig: c });
      break;
    case 'CHANGE_FEATURE_MODE':
      if (c.features?.[msg.key as string]) c.features[msg.key as string].mode = msg.mode;
      await chrome.storage.sync.set({ extensionConfig: c });
      break;
    case 'RESET_CONFIG':
      ...
      break;
    case 'ADD_URL': {
      const u = (await chrome.storage.sync.get('extensionCustomUrls')) as {
    ...
    case 'DELETE_URL': {
      const d = (await chrome.storage.sync.get('extensionCustomUrls')) as {
        extensionCustomUrls?: CustomUrl[];
      };
    ...
    case 'TOGGLE_URL': {
      const t = (await chrome.storage.sync.get('extensionCustomUrls')) as {
        extensionCustomUrls?: CustomUrl[];
      };
      const turls = t.extensionCustomUrls || [];
      turls.forEach((x) => {
        if (x.id === msg.id) x.enabled = msg.enabled as boolean;
      });
...
function renderFeatures(): void {
  ...
  for (const [key, feature] of Object.entries(features)) {
    ...
    if (!isComingSoon) {
      ...
      if (key === 'openDetailInNewTab' && feature.modes && feature.enabled) {
        ...
        modeHtml =
          ...
          '<select class="feature-mode-select" data-feature="' +
          key +
          '" style="padding:4px 8px;border-radius:4px;border:1px solid #d1d5db;font-size:11px;">' +
          Object.entries(feature.modes)
            .map(function (m) {
              return (
                '<option value="' +
                m[0] +
                '"' +
                (m[0] === currentMode ? ' selected' : '') +
                '>' +
                m[1] +
                '</option>'
              );
      ...
      controlsHtml =
        ...
        '<input type="checkbox" class="toggle-checkbox feature-toggle" data-feature="' +
        key +
        '"' +
    ...
    div.innerHTML =
      ...
      (feature.name || key) +
      (isComingSoon
        ? ' <span style="color:#ef4444;font-size:9px;font-weight:bold;">(COMING SOON)</span>'
        : '') +
      '</span>' +
      '<span class="subtitle">' +
      (feature.description || '') +
  ...
  if (!hasFeatures) {
    ...
    div.innerHTML =
      '<div style="font-size:13px;">Tidak ada fitur untuk role: <strong>' +
      role +
      '</strong></div>' +
...
function renderUrls(): void {
  ...
  customUrls.forEach(function (url) {
    ...
    item.innerHTML =
      ...
      url.id +
      '">' +
      '<button class="btn-delete" data-url-id="' +
      url.id +
      '"' +
      (url.isDefault ? ' disabled' : '') +
      '>Hapus</button>' +
      '</div>';
...
async function addNewUrl(): Promise<void> {
  ...
  try {
    ...
    DOM.featuresList.addEventListener('change', (e) => {
      ...
      if (target.classList.contains('feature-toggle')) {
        ...
        DOM.featuresList
          .querySelectorAll(`.feature-toggle[data-feature="${key}"]`)
          .forEach((el) => {
        ...
        const key = (target as HTMLSelectElement).dataset.feature as string;
        bgWrite({ type: 'CHANGE_FEATURE_MODE', key, mode: (target as HTMLSelectElement).value });
    ...
    DOM.roleSelect.addEventListener('change', async (e) => {
      ...
      await bgWrite({ type: 'SET_ROLE', role: target.value as Role });
      if (currentConfig) currentConfig.currentRole = target.value as Role;
      updateRoleBanner();
      renderFeatures();
    ...
    DOM.roleBanner.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).id === 'changeRoleLink') {
        e.preventDefault();
