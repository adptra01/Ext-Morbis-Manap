/**
 * telaahResepPrint — restyle halaman cetak "Telaah Resep" MORBIS
 * (/inventory/print/telaah-resep-manap?id_resep=...) agar persis mengikuti
 * template "Resep Medis - RSUD H. Abdul Manap": 2 kolom, kartu metadata abu,
 * daftar obat dgn Jml di kanan, tabel checklist rapi, print A5 12.5cm×11.5cm.
 * TANPA menghapus/mereduksi informasi.
 *
 * CSS SELF-CONTAINED (bukan Tailwind CDN) → layout 2 kolom dijamin jalan,
 * offline-safe, bebas masalah CSP/scan CDN. Font Inter dari Google Fonts
 * (fallback Arial tetap terbaca bila offline).
 *
 * Jalan di world:MAIN. Halaman ini murni dokumen cetak (tanpa input data)
 * → rebuild struktur aman. Semua data dibaca ulang dari DOM asli → tidak hilang.
 * Cetak tetap lewat `cetak()` MORBIS (tombol disertakan, class `no-print`).
 */
(function () {
  'use strict';
