...
function rowToData(
  ths: NodeListOf<HTMLTableCellElement>,
...
<div class="ext-modal-body"><div class="ext-modal-grid">${fields}</div></div>
  ...
  $(document)
    ...
    .on('keydown.extModal', function (e: KeyboardEvent) {
  ...
  root.addEventListener('click', (e) => {
    ...
    if (trigger) {
      ...
      if (!isOpen) {
        ...
        const dd = menu.querySelector<HTMLElement>('.ext-action-dropdown');
...
function initTable(table: HTMLTableElement, config: (typeof TABLE_CONFIGS)[0]): void {
  ...
  const colDefs: any[] = [{ targets: 0, width: '40px', orderable: false, className: 'dt-center' }];
...
function scanTables(): void {
  TABLE_CONFIGS.forEach((cfg) => {
    ...
    if (el && !(el as HTMLElement).dataset.extDt) {
      initTable(el as HTMLTableElement, cfg);
    }
  });
}
    ...
    loadDataTablesDeps(LOG).then(() => {
      ...
      (function wait() {
        ...
        const anyPending = TABLE_CONFIGS.some((c) => {
          ...
          return el && (el as HTMLElement).dataset.extDt !== '1';
        });
