...
function getTargetSection(): HTMLElement | null {
  return (
    document.getElementById('rincian-biaya-view') || document.getElementById('pembayaran-gabung')
  );
}

function getAllBillingTbodies(): { tindakan: HTMLElement | null; obat: HTMLElement | null } | null {
...
interface ResepSection {
  label: string;
  subtotal: Subtotal | null;
}

function parseTindakanSections(tbody: HTMLElement): Section[] {
  ...
  for (const row of rows) {
    ...
    if (bigColCell) {
      ...
      if (
        ...
        !unitName.match(/^\d/) &&
        unitName.length > 0 &&
        !unitName.toLowerCase().includes('cetak oleh')
      ) {
...
function parseResepSections(tbody: HTMLElement): ResepSection[] {
  const sections: ResepSection[] = [];
  let currentResep: ResepSection | null = null;
  const rows = Array.from(tbody.querySelectorAll('tr'));
...
function createTotalRow(label: string, totalObj: Subtotal, isObat = false): HTMLTableRowElement {
  const tr = document.createElement('tr');
  tr.dataset.extSummary = 'true';
...
function applyRingkasMode(
  tbodies: { tindakan: HTMLElement | null; obat: HTMLElement | null } | null,
): void {
  Array.from(document.querySelectorAll('[data-ext-summary]')).forEach((el) => el.remove());
  Array.from(document.querySelectorAll('.ext-header-hidden')).forEach((el) =>
    el.classList.remove('ext-header-hidden'),
  );
  if (!tbodies) return;
  ...
  if (tbodies.tindakan) {
    ...
    rows.forEach((r) => {
      ...
      const isHeader =
        r.classList.contains('tabel-label') ||
        r.dataset.extSummary ||
        (txt.includes('BIAYA') && txt.includes('RINCIAN'));
    ...
    const totalA: Subtotal = { tarif: 0, tunai: 0, jaminan: 0 };
    ...
    const anchor = rows.find(
      (r) =>
        (r.textContent?.includes('Total Jasa') || r.textContent?.includes('Jasa Pelayanan')) &&
        !r.dataset.extSummary,
    );
    if (anchor) {
  ...
  if (tbodies.obat) {
    ...
    const totalB: Subtotal = { tarif: 0, tunai: 0, jaminan: 0 };
    ...
    const anchor = rows.find(
      (r) =>
        footerKeywords.some((keyword) => (r.textContent || '').includes(keyword)) &&
        !r.dataset.extSummary,
    );
    if (anchor) {
...
function applyPenuhMode(): void {
  Array.from(document.querySelectorAll('[data-ext-summary]')).forEach((el) => el.remove());
  Array.from(document.querySelectorAll('.ext-header-hidden')).forEach((el) =>
    el.classList.remove('ext-header-hidden'),
  );
  Array.from(document.querySelectorAll('tr.ext-billing-hidden')).forEach((el) =>
    el.classList.remove('ext-billing-hidden'),
  );
  const section = getTargetSection();
  if (section) {
    const titleEl = section.querySelector('u');
...
function renderToggleButton(section: HTMLElement): void {
  ...
  btn.style.cssText =
    'margin: 8px 0 4px 10px; padding: 5px 14px; background: #6366f1; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 12px; font-weight: 600; display: block;';

  const updateLabel = () => {
    btn.textContent =
      billingViewMode === 'ringkas' ? '📄 Tampilkan Rincian Penuh' : '📋 Ringkaskan Rincian Biaya';
  ...
  updateLabel();
  const heading = section.querySelector('.panel-heading') as HTMLElement | null;
  if (heading) {
...
function runSimplifyBillingFeature(): void {
  if (
    !g.currentConfig?.features?.simplifyBilling?.enabled ||
    !g.ExtensionCore.isFeatureAllowed('simplifyBilling')
  )
    return;
