...
/** Cetak kartu antrian hari ini (dari app) — format kartu termal SAMA dengan
 *  cetak kosong: satu kartu per antrian + nama pasien, page-break per kartu
 *  → printer termal memotong sendiri. */
  ...
  const cards = rows
    .map((r) => {
      ...
      return (
        ...
        `<div style="font-size:110px;font-weight:900;letter-spacing:-2px;line-height:1;">${r.queue_number}</div>` +
        '</div>' +
        `<div style="font-size:20px;font-weight:bold;margin-top:10px;">${r.nama_pasien || '-'}</div>` +
        jenisLine +
    ...
    .join('');
  win.document.write(
    '<html><head><title>Antrian Farmasi — Kartu</title>' +
...
function buildPanel(): HTMLDivElement {
  ...
  p.style.cssText =
    ...
    'background:#f8f9fa;height:100vh;box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;';
  p.innerHTML =
    ...
    '<div id="ext-col-panel" style="background:#fff;border:1px solid #dee2e6;border-radius:16px;padding:12px;min-width:0;min-height:0;display:flex;flex-direction:column;overflow:hidden;"></div>' +
    '</div>';
  return p;
}
