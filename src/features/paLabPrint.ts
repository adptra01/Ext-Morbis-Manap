(function () {
  ...
  async function apply() {
    ...
    // Normalisasi nama RS di titik ekstraksi (kop, info pasien, export
    // Word):
    //   1) gelar salah "KH." (Kiai Haji; default server saat field
    //      "RS Luar" kosong) → "H." (Haji): "RSUD KH. Abdul Manap"
    //      → "RSUD H. Abdul Manap".
    //   2) spasi setelah gelar: "H.ABDUL MANAP" (bentuk kop server
    //      tanpa spasi) → "H. ABDUL MANAP".
    // Pola dibatasi: gelar (KH/H) yang langsung diikuti "Abdul Manap"
    // (nama RS ini) — teks medis lain tidak tersentuh.
