...
(function () {
  ...
  // Loading overlay singkat saat user klik nomor antrian — menutupi transisi
  // native→CSS (server simpan→ws→reload) supaya tidak terasa "kedip" halaman lama.
  // ponytail: sessionStorage supaya bertahan jika halaman reload, hide otomatis saat
  // UI custom siap / timeout 5s (guard native/fallback bila antrianTools tidak inject).
  function showTicketLoading(): void {
    try {
    ...
    l.style.cssText =
      ...
      'background:#D5E9DB;font-family:Inter,"Segoe UI",system-ui,sans-serif;transition:opacity .25s;';
    l.innerHTML =
      ...
      '<div style="width:40%;height:100%;border-radius:999px;background:#0f5132;animation:ticket-spin 1.1s ease-in-out infinite;"></div>' +
      '</div>' +
      '<p style="margin:0;color:#495057;font-size:15px;font-weight:600;">Menyiapkan antrian Anda…</p>' +
      '</div>';
    document.body.appendChild(l);
    hideTicketLoadingAfter(5000); // guard: kalau UI tak muncul (native/fallback), tutup overlay
  }

  function hideTicketLoading(): void {
  ...
  function hideTicketLoadingAfter(ms: number): void {
    setTimeout(() => {
  ...
  function init(): void {
    ...
    const renderMesinUI = () => {
      ...
      setHealth('ui');
      // loading selesai: UI custom sudah siap (kasus reload setelah klik antrian) — tutup overlay
      if (sessionStorage.getItem('ext-m-ticket-loading') === '1') hideTicketLoading();
    };
