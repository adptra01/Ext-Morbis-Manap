(function () {
  ...
  function simplifyMesinAntrian(): void {
    ...
    setTimeout(function () {
      const h3 = document.querySelector('.card1 .card-body h3') as HTMLElement | null;
      if (h3) {
        h3.textContent = 'ANTRIAN RUMAH SAKIT RSUD ABDUL MANAP';
        h3.style.fontSize = '22px';
      }
      const card = document.querySelector('.card1 .card') as HTMLElement | null;
      if (card) {
  ...
  function simplifyViewAntrian(): void {
    ...
    function hideExtraSliders(): void {
      ...
      if (allCards.length > 1) {
        for (var j = 1; j < allCards.length; j++) {
          (allCards[j] as HTMLElement).style.display = 'none';
        }
      }
      if (allCards.length >= 1) {
        var firstCard = allCards[0] as HTMLElement;
      ...
      for (var k = 0; k < queueNames.length; k++) {
        var el = queueNames[k] as HTMLElement;
    ...
    for (var r = 1; r <= 6; r++) {
      setTimeout(hideExtraSliders, r * 3000);
    }
  }
  ...
  function fixCounter(): void {
    ...
    var poll = setInterval(function () {
      ...
      if (resetBtn) {
        ...
        clearInterval(poll);
        fixResetButton(resetBtn as HTMLButtonElement);
      }
    }, 300);
  ...
  function pollCounterSimplify(retries: number): void {
    if (retries <= 0) return;
    setTimeout(function () {
      ...
      if (container) {
        ...
        if (cards.length > 1) {
          ...
          (cards[0] as HTMLElement).style.maxWidth = '600px';
          (cards[0] as HTMLElement).style.margin = '0 auto';
        }
      } else {
  ...
  function fixResetButton(resetBtn: HTMLButtonElement): void {
    resetBtn.onclick = function (e: Event) {
      ...
      var swal = typeof w.swal === 'function' ? (w.swal as Function) : null;
      ...
      var doReset = function () {
        ...
        xhr.onload = function () {
          try {
            ...
            if (data.status === 200) {
              if (swal) {
                swal({ title: 'Reset Berhasil', text: 'Antrian dikosongkan. Memuat ulang...', icon: 'success', timer: 2000 });
              }
