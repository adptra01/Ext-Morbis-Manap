...
const CPPT_NO_RESULTS_CLASS = 'ext-cppt-no-results';
...
function isCpptTable(table: HTMLTableElement): boolean {
...
function findCpptTables(): HTMLTableElement[] {
  return Array.from(document.querySelectorAll<HTMLTableElement>('table')).filter(isCpptTable);
}
...
function normalizeDateForCompare(s: string): string {
  const m = s.match(/(\d{2})\/(\d{2})\/(\d{4})/);
  if (m) return m[3] + '-' + m[2] + '-' + m[1];
  return s;
}

function getStorageKey(tableIdx: number): string {
  return 'ext_cppt_filter_' + tableIdx;
}

function getUrlPrefix(tableIdx: number): string {
  return 'cppt' + tableIdx + '_';
}

function getStoredFilters(tableIdx: number): CpptFilterState {
  try {
...
function syncUrlParams(state: CpptFilterState, tableIdx: number): void {
...
function injectFilterForTable(table: HTMLTableElement, tableIdx: number): void {
