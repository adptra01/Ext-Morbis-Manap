(function () {
  ...
  function showLoadError(msg) {
    ...
    content.innerHTML =
      '<div class="ext-error">' +
      '<p class="ext-error-title">Gagal memuat data resume</p>' +
      '<p>' + esc(msg) + '</p>' +
      '<button id="ext-error-back" class="ext-btn ext-btn-danger">Kembali</button>' +
      '</div>';
    var backBtn = content.querySelector('#ext-error-back');
    if (backBtn) backBtn.addEventListener('click', function () { closeOverlay(); });
  }
  ...
  function loadResumeData(idVisit) {
    ...
    fetch(url, { credentials: 'same-origin' })
      ...
      .then(function (html) { parseAndRender(html, idVisit); })
      .catch(function (e) {
  ...
  function extractFormData(doc) {
    ...
    // Capture ALL form elements (input, select, textarea) from original form
    // Group by name: single values OR arrays for name[] fields
    var allFormData = {};
    var arrayFields = {}; // name -> [val1, val2, ...]
    ...
    if (form) {
      ...
      for (var i = 0; i < elements.length; i++) {
        var el = elements[i];
        if (!el.name) continue;
        // Skip checkboxes/radios that aren't checked
    ...
    // Also capture hidden inputs separately (in case form is null)
    ...
    // Extract vital signs explicitly (single values)
    ...
    // Add array fields to base (as arrays for serialization)
    for (var arrKey in arrayFields) {
      base[arrKey] = arrayFields[arrKey];
    }

    // Handle codeProsedur[] - map from kode9[] if missing (server expects array format)
  ...
  function extractDiagnosaRows(doc) {
    ...
    idicdInputs.forEach(function (inp) {
      ...
      rows.push({ idicd: id, kode: kode, nama: nama, kasus_diagnosa: kasus, komplikasi: komp });
    });
    return rows;
  }
  ...
  function buildFormHTML(data) {
    return '' +
      '<form id="ext-resume-form">' +
      buildHiddenFields(data) +
      buildInfoSection(data) +
      buildClinicalSection(data) +
      buildDiagnosaSection(data) +
      buildTindakanSection(data) +
      buildActionButtons() +
      '</form>';
  }
  ...
  function buildHiddenFields(data) {
    ...
    // Extra hidden fields from original form (single-value, non-array)
    var skipKeys = {
      id_visit:1, id_rawat_jalan:1, id_user:1, id_dokter:1, id_bed:1,
      noregis:1, norm:1, pasien:1, nama_dokter:1, waktu:1,
      alergiMakananJSON:1, alergiLingkunganJSON:1,
      tensi:1, nadi:1, suhu:1, nafas:1, tinggi:1, berat:1,
      anamnesa:1, pemeriksaan_fisik:1, catatan:1, tindakan:1,
      terapi_pengobatan:1, composition_diet:1,
      save:1
    };
    for (var key in data) {
  ...
  function buildInfoSection(data) {
    return '' +
      ...
      '<div class="ext-info-item"><label>No. RM</label><div class="ext-info-value">' + esc(data.norm) + '</div></div>' +
      '<div class="ext-info-item ext-info-item-wide"><label>Nama Pasien</label><div class="ext-info-value">' + esc(data.pasien) + '</div></div>' +
      '<div class="ext-info-item ext-info-item-wide"><label>Dokter DPJP</label><div class="ext-info-value">' + esc(data.nama_dokter) + '</div></div>' +
      '</div>';
  }
  ...
  function buildClinicalSection(data) {
    return '' +
      ...
      '<h3 class="ext-section-title">Data Klinis</h3>' +
      '<div class="ext-clinical-grid">' +
      buildTextarea('anamnesa', 'Anamnesa', data.anamnesa) +
      buildTextarea('pemeriksaan_fisik', 'Pemeriksaan Fisik', data.pemeriksaan_fisik) +
      buildTextarea('catatan', 'Diagnosa (Catatan) *', data.catatan) +
      buildTextarea('tindakan', 'Tindakan', data.tindakan) +
      buildTextarea('terapi_pengobatan', 'Terapi Pengobatan', data.terapi_pengobatan) +
      '</div>' +
      '<h3 class="ext-section-title" style="margin-top:16px;">Tanda Vital</h3>' +
      '<div class="ext-clinical-grid">' +
      buildTextInput('tensi', 'Tensi (mmHg)', data.tensi) +
      buildTextInput('nadi', 'Nadi (x/menit)', data.nadi) +
      buildTextInput('suhu', 'Suhu (°C)', data.suhu) +
      buildTextInput('nafas', 'Nafas (x/menit)', data.nafas) +
      buildTextInput('tinggi', 'Tinggi Badan (cm)', data.tinggi) +
      buildTextInput('berat', 'Berat Badan (kg)', data.berat) +
      '</div></div>';
  }
  ...
  function buildTextarea(name, label, value) {
    return '' +
      '<div class="ext-field">' +
      '<label for="ext-' + name + '">' + esc(label) + '</label>' +
      '<textarea name="' + name + '" id="ext-' + name + '">' + esc(value) + '</textarea>' +
      '</div>';
  }
  ...
  function buildTextInput(name, label, value) {
    return '' +
      '<div class="ext-field">' +
      '<label for="ext-' + name + '">' + esc(label) + '</label>' +
      '<input type="text" name="' + name + '" id="ext-' + name + '" value="' + esc(value) + '">' +
      '</div>';
  }

  function buildDiagnosaSection(data) {
    var html = '<div class="ext-section">' +
      '<div class="ext-section-header">' +
      '<h3 class="ext-section-title">Diagnosa (ICD-10)</h3>' +
      '<button type="button" id="ext-add-diagnosa" class="ext-btn ext-btn-primary ext-btn-sm">+ Tambah Diagnosa</button>' +
      '</div>' +
      '<table class="ext-table"><thead><tr>' +
      '<th style="width:35%;">Nama Diagnosa</th>' +
      '<th style="width:12%;">Kode ICD-10</th>' +
      '<th style="width:15%;">Kasus</th>' +
      '<th style="width:13%;">Komplikasi</th>' +
      '<th style="width:8%;">Aksi</th>' +
      '</tr></thead>' +
      '<tbody id="ext-diagnosa-tbody">';
  ...
  function buildDiagnosaRowHTML(index, data) {
    ...
    return '' +
      '<tr data-row="' + index + '">' +
      '<td><input type="text" name="nama[]" value="' + esc(data.nama) + '" id="nama' + index + '" class="ext-ac-icd10-nama" placeholder="Cari diagnosa..."></td>' +
      '<td><input type="text" name="kode10[]" value="' + esc(data.kode) + '" id="kode' + index + '" class="ext-ac-icd10-kode" placeholder="Kode ICD-10"></td>' +
      '<input type="hidden" name="idicd[]" value="' + esc(data.idicd) + '" id="idicd' + index + '">' +
      '<td><select name="kasus_diagnosa[]">' + kasusOpts + '</select></td>' +
      '<td><select name="komplikasi[]">' + kompOpts + '</select></td>' +
      '<td><button type="button" class="ext-rm-row ext-btn-danger">Hapus</button></td>' +
      '</tr>';
  }
  ...
  function buildTindakanSection(data) {
    var html = '<div class="ext-section">' +
      '<div class="ext-section-header">' +
      '<h3 class="ext-section-title">Tindakan (ICD-9)</h3>' +
      '<button type="button" id="ext-add-tindakan" class="ext-btn ext-btn-primary ext-btn-sm">+ Tambah Tindakan</button>' +
      '</div>' +
      '<table class="ext-table"><thead><tr>' +
      '<th style="width:30%;">Nama Tindakan</th>' +
      '<th style="width:10%;">Kode ICD-9</th>' +
      '<th style="width:12%;">Komorbid</th>' +
      '<th style="width:15%;">Kategori Prosedur</th>' +
      '<th style="width:8%;">Aksi</th>' +
      '</tr></thead>' +
      '<tbody id="ext-tindakan-tbody">';
  ...
  function buildTindakanRowHTML(index, data) {
    ...
    return '' +
      '<tr data-row="' + index + '">' +
      '<td><input type="text" name="namaTindakan[]" value="' + esc(data.namaTindakan) + '" id="namaTindakan' + index + '" class="ext-ac-icd9-nama" placeholder="Cari tindakan..."></td>' +
      '<td><input type="text" name="kode9[]" value="' + esc(data.kode9) + '" id="kode9' + index + '" class="ext-ac-icd9-kode" placeholder="Kode ICD-9"></td>' +
      '<input type="hidden" name="idicdTindakan[]" value="' + esc(data.idicdTindakan) + '" id="idicdTindakan' + index + '">' +
      '<input type="hidden" name="snomedProsedur[]" value="' + esc(data.snomedProsedur) + '" id="snomedProsedur' + index + '">' +
      '<input type="hidden" name="codeProsedur[]" value="' + esc(data.codeProsedur) + '" id="codeProsedur' + index + '">' +
      '<td><select name="komorbid[]">' + komorbidOpts + '</select></td>' +
      '<td><select name="kategoriProsedur[]" class="ext-kategori-prosedur">' + kategoriOpts + '</select></td>' +
      '<td><button type="button" class="ext-rm-row ext-btn-danger">Hapus</button></td>' +
      '</tr>';
  }
  ...
  function buildActionButtons() {
    return '' +
      ...
      '<button type="button" id="ext-resume-simpan" class="ext-btn ext-btn-success">Simpan Resume</button>' +
      '<button type="button" id="ext-resume-cancel" class="ext-btn ext-btn-secondary">Tutup</button>' +
      '<span id="ext-resume-status" class="ext-status"></span>' +
      '</div>';
  }

  function bindDynamicRows(root) {
    var addDiag = root.getElementById('ext-add-diagnosa');
    if (addDiag) addDiag.addEventListener('click', function () { addDiagnosaRow(root); });
    var addTind = root.getElementById('ext-add-tindakan');
    if (addTind) addTind.addEventListener('click', function () { addTindakanRow(root); });
  ...
  function serializeForm(form) {
    // Group elements by name to handle arrays properly
    var nameToElements = {};
    ...
    // Build serialized data array of [key, value] pairs for proper array handling
    var pairs = [];
    for (var name in nameToElements) {
      var elements = nameToElements[name];
      if (name.endsWith('[]') || elements.length > 1) {
        // Array field: each element gets same name with []
  ...
  function doSave(btn, form) {
    ...
    fetch(ENDPOINT, {
      ...
      .catch(function (e) { afterSaveError(btn, e.message); });
  }
  ...
  function bindCancel(root) {
    ...
    if (cancelBtn) cancelBtn.addEventListener('click', function () { closeOverlay(); });
  }
  ...
  function setupAcInput(input, url, onSelect) {
    ...
    input.addEventListener('input', function () {
      ...
      if (val.length < 2) { menu.style.display = 'none'; return; }
      fetch(url + '&q=' + encodeURIComponent(val), { credentials: 'same-origin' })
        .then(function (r) { return r.json(); })
        .then(function (data) {
          menu.innerHTML = '';
          if (!data || data.length === 0) { menu.style.display = 'none'; return; }
  ...
  function getShadowHTML() {
    return '' +
      '<div class="ext-root">' +
      '<button id="ext-floating-btn" class="ext-float-btn" title="Resume Rajal">RJ</button>' +
      '<div class="ext-overlay">' +
      '<div class="ext-backdrop"></div>' +
      '<div class="ext-panel">' +
      '<div class="ext-panel-header">' +
      '<span class="ext-panel-title">Resume Rajal</span>' +
      '<button id="ext-overlay-close" class="ext-close-btn">&times;</button>' +
      '</div>' +
      '<div class="ext-panel-body" id="ext-resume-content">' +
      '<div class="ext-loading">' +
      '<div class="ext-spinner"></div>' +
      '<p>Klik tombol RJ untuk memuat form resume...</p>' +
      '</div>' +
      '</div>' +
      '</div>' +
      '</div>' +
  ...
  function getShadowCSS() {
    return '' +
      '<style>' +
      '.ext-root { all: initial; display: block; }' +
      '' +
      '.ext-float-btn {' +
      'position: fixed; right: 16px; top: 50%; transform: translateY(-50%);' +
      'z-index: 2147483647; width: 48px; height: 48px; border-radius: 50%;' +
      ...
      '}' +
      '.ext-float-btn:hover { transform: translateY(-50%) scale(1.1); box-shadow: 0 6px 24px rgba(14,165,233,.5); }' +
      '' +
      '.ext-overlay {' +
      ...
      'font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;' +
      '}' +
      '.ext-backdrop { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,.4); }' +
      '' +
      '.ext-panel {' +
      'position: relative; width: 800px; max-width: 94vw; max-height: 90vh;' +
      ...
      '}' +
      '.ext-panel-title { font-size: 16px; font-weight: 700; color: #0c4a6e; }' +
      '.ext-close-btn {' +
      'background: none; border: none; font-size: 24px; color: #94a3b8; cursor: pointer;' +
      'padding: 0 4px; line-height: 1;' +
      '}' +
      '.ext-close-btn:hover { color: #475569; }' +
      '' +
      '.ext-panel-body {' +
      ...
      '}' +
      '' +
      '.ext-loading { text-align: center; padding: 60px 20px; color: #6b7280; }' +
      '.ext-spinner {' +
      'display: inline-block; width: 36px; height: 36px;' +
      'border: 3px solid #e5e7eb; border-top-color: #0ea5e9; border-radius: 50%;' +
      'animation: ext-spin .8s linear infinite;' +
      '}' +
      '' +
      '.ext-error { text-align: center; padding: 40px 20px; }' +
      '.ext-error-title { font-size: 16px; font-weight: 600; color: #dc2626; margin: 0 0 8px; }' +
      '' +
      '.ext-info-box {' +
      'display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 20px;' +
      'padding: 14px 16px; background: #f0f9ff; border-radius: 8px; border-left: 4px solid #0ea5e9;' +
      '}' +
      '.ext-info-item { flex: 1; min-width: 140px; }' +
      '.ext-info-item-wide { flex: 2; min-width: 200px; }' +
      '.ext-info-item label { font-weight: 600; color: #0c4a6e; font-size: 11px; display: block; margin-bottom: 4px; text-transform: uppercase; letter-spacing: .5px; }' +
      '.ext-info-value { padding: 6px 10px; background: #fff; border-radius: 4px; font-size: 13px; border: 1px solid #bae6fd; }' +
      '' +
      '.ext-section { margin-bottom: 18px; }' +
      '.ext-section-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }' +
      '.ext-section-title { color: #0369a1; font-size: 14px; margin: 0; padding-bottom: 6px; border-bottom: 2px solid #e0f2fe; }' +
      '' +
      '.ext-clinical-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }' +
      '.ext-field { grid-column: span 1; }' +
      '.ext-field label { font-weight: 600; color: #334155; font-size: 11px; display: block; margin-bottom: 3px; }' +
      '.ext-field textarea {' +
      'width: 100%; min-height: 64px; padding: 7px 9px;' +
      'border: 1px solid #cbd5e1; border-radius: 4px; font-size: 13px;' +
      'resize: vertical; font-family: inherit; box-sizing: border-box;' +
      '}' +
      '.ext-field textarea:focus { outline: none; border-color: #0ea5e9; box-shadow: 0 0 0 3px rgba(14,165,233,.15); }' +
      '' +
      '.ext-table { width: 100%; border-collapse: collapse; font-size: 13px; }' +
      '.ext-table th { padding: 7px 8px; text-align: left; border: 1px solid #e2e8f0; background: #f0f9ff; font-weight: 600; color: #0c4a6e; font-size: 11px; text-transform: uppercase; letter-spacing: .5px; }' +
      '.ext-table td { padding: 5px 6px; border: 1px solid #e2e8f0; }' +
      '.ext-table input, .ext-table select {' +
      'width: 100%; padding: 4px 7px; border: 1px solid #cbd5e1; border-radius: 4px;' +
      'font-size: 13px; box-sizing: border-box; font-family: inherit;' +
      '}' +
      '.ext-table input:focus, .ext-table select:focus { outline: none; border-color: #0ea5e9; box-shadow: 0 0 0 3px rgba(14,165,233,.15); }' +
      '' +
      '.ext-actions {' +
      'display: flex; gap: 10px; margin-top: 18px; padding-top: 14px;' +
      'border-top: 2px solid #e2e8f0; align-items: center;' +
      '}' +
      '' +
      '.ext-btn {' +
      'padding: 9px 24px; border: none; border-radius: 6px; font-size: 13px;' +
      'font-weight: 600; cursor: pointer; font-family: inherit;' +
      'transition: opacity .2s, background .2s;' +
      '}' +
      '.ext-btn:disabled { opacity: .5; cursor: not-allowed; }' +
      '.ext-btn-success { background: #16a34a; color: #fff; }' +
      '.ext-btn-success:hover:not(:disabled) { background: #15803d; }' +
      '.ext-btn-secondary { background: #6b7280; color: #fff; }' +
      '.ext-btn-secondary:hover:not(:disabled) { background: #4b5563; }' +
      '.ext-btn-primary { background: #0ea5e9; color: #fff; }' +
      '.ext-btn-primary:hover:not(:disabled) { background: #0284c7; }' +
      '.ext-btn-danger { background: #ef4444; color: #fff; }' +
      '.ext-btn-sm { padding: 5px 12px; font-size: 12px; }' +
      '' +
      '.ext-status { display: none; padding: 6px 12px; border-radius: 4px; font-size: 12px; font-weight: 600; }' +
      '' +
      '.ext-ac-item {' +
      'padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #f3f4f6;' +
      'transition: background .15s;' +
      '}' +
      '.ext-ac-item:hover { background: #f0f9ff; }' +
      '.ext-ac-kode { color: #6b7280; }' +
      '' +
      '@keyframes ext-spin { to { transform: rotate(360deg); } }' +
      '' +
      '@media print {' +
      '  .ext-float-btn, .ext-overlay { display: none !important; }' +
      '}' +
      '</style>';
  }
