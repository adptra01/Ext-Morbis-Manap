...
function injectPrintOptimizationStyles(): void {
...
function isEffectivelyEmpty(section: HTMLElement): boolean {
  const hasText = section.textContent?.trim().length > 0;

  const hasVisuals =
    section.querySelectorAll('img, canvas, svg, iframe, video, figure, picture, object, embed')
      .length > 0;
  ...
  const hasFilledInputs = Array.from(
    ...
    (el: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement) => el.value?.trim() !== '',
  );

  return !hasText && !hasVisuals && !hasTableWithRows && !hasFilledInputs;
}

const INLINE_STYLE_PROPS: Array<[string, string]> = [
...
function removeInlineStyles(): void {
  ...
  targets.forEach((el) => {
    INLINE_STYLE_PROPS.forEach(([jsKey, dataKey]) => {
      const styleVal = (el.style as Record<string, string>)[jsKey];
      if (styleVal) {
        el.dataset[dataKey] = styleVal;
        if (jsKey.startsWith('pageBreak') || jsKey.startsWith('break')) {
          (el.style as Record<string, string>)[jsKey] = 'auto';
        } else if (jsKey === 'overflow' || jsKey === 'overflowY' || jsKey === 'overflowX') {
          (el.style as Record<string, string>)[jsKey] = 'visible';
        } else if (jsKey === 'height' || jsKey === 'maxHeight' || jsKey === 'minHeight') {
          (el.style as Record<string, string>)[jsKey] = '';
        } else {
          (el.style as Record<string, string>)[jsKey] = '0';
        }
      }
    });
  });
  ...
  targets.forEach((el) => {
    INLINE_STYLE_PROPS.forEach(([jsKey, dataKey]) => {
      if (el.dataset[dataKey] !== undefined) {
        (el.style as Record<string, string>)[jsKey] = el.dataset[dataKey] || '';
...
function avoidBreakOnFitSections(): void {
  document.querySelectorAll<HTMLElement>('.isidalam').forEach((el) => {
...
function restoreBreakOnFitSections(): void {
  document.querySelectorAll<HTMLElement>('.isidalam').forEach((el) => {
...
function hideSectionsBeforePrint(): void {
  try {
    removeInlineStyles();
    avoidBreakOnFitSections();

    const sections = document.querySelectorAll<HTMLElement>(PRINT_OPT_CONFIG.selectors);
    let hiddenCount = 0;
...
function restoreSectionsAfterPrint(): void {
  try {
    restoreInlineStyles();
    ...
    document.querySelectorAll<HTMLElement>('.isidalam[data-keep-together]').forEach((el) => {
...
let eagerObserver: MutationObserver | null = null;

function eagerCleanBreakStyles(root?: HTMLElement): void {
  (root || document).querySelectorAll<HTMLElement>('.isidalam').forEach((el) => {
...
function runPrintOptimization(): void {
  if (typeof g.currentConfig === 'undefined' || typeof g.featureModules === 'undefined') return;

  const featureEnabled =
    g.currentConfig?.features?.printOptimization?.enabled &&
    g.ExtensionCore.isFeatureAllowed('printOptimization');
      ...
      for (const node of Array.from(m.addedNodes)) {
        if (node.nodeType === 1) {
          ...
          if (el.matches && el.matches('.isidalam')) {
            eagerCleanBreakStyles(el.parentElement || undefined);
            return;
          }
