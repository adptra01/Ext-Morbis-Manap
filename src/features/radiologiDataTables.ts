 ...
 * Tabs: Belum Periksa (belum) & Sudah Periksa (sudah) loaded via contentloader AJAX.
 *
 * Features:
...
/** Row id used by the page's own action JS (edit_pemeriksaan etc.). */
function getRowId(rowData: any): string {
  const id = rowData.id ?? rowData.no_registrasi ?? rowData.no_rm ?? '';
...
<div class="ext-action-menu" tabindex="0" role="menu" aria-label="Aksi untuk ${rowData.no_rm || ''} ${rowData.nama || ''}" aria-expanded="false">
  ...
  <div class="ext-action-dropdown" role="menu">
    ${menuItems
      .map(
        (item) => `
      <a role="menuitem" tabindex="-1" href="#" onclick="extRadioAction('${item.fn}', '${id}'); return false;">
        ${item.label}
      </a>
    `,
      )
...
function createStatusBadge(status: string): string {
  ...
  if (s.includes('lunas') && !s.includes('belum')) cls = 'ext-badge--sudah';
  else if (s.includes('belum')) cls = 'ext-badge--belum';
...
(window as any).extRadioAction = function (fnName: string, id: string): void {
...
// ─── Modal Detail ────────────────────────────────────────────────────────
(window as any).extRadioShowDetail = function (rowData: any): void {
  const $ = getExt$();
  if (!$) return;
...
<div class="ext-modal-overlay" tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="ext-modal-title">
  <div class="ext-modal">
    <div class="ext-modal-header">
      <h3 class="ext-modal-title" id="ext-modal-title">Detail Pemeriksaan Radiologi — ${rowData.no_rm} ${rowData.nama}</h3>
  ...
  $(document).on('keydown.extRadioModal', function (e: KeyboardEvent) {
...
function bindDropdownToggle(): void {
  if ((document as any).__extRadioDtToggleBound) return;
  (document as any).__extRadioDtToggleBound = true;
  ...
  try {
    dtInstance = $(table).DataTable({
      ...
      columns: [
        ...
        { data: 'status_pembayaran', render: createStatusBadge },
      ...
      order: [[2, 'desc']], // Default sort by tgl masuk desc
      scrollX: true,
      fixedHeader: true,
      autoWidth: false,
        ...
        $row
          ...
          .on('click keyup', function (this: HTMLElement, e: Event) {
            ...
            if (e.type === 'click' || ke.key === 'Enter' || ke.key === ' ') {
              e.preventDefault();
              (window as any).extRadioShowDetail(data);
            }
          });
      },
...
function initClientSide(table: HTMLTableElement, tab: TabKey): void {
  ...
  dtInstance = $(table).DataTable({
    ...
    columnDefs: [
      ...
      { targets: 2, width: '140px' }, // Tgl Masuk
      { targets: 3, width: '250px' }, // Tindakan Pemeriksaan
      { targets: 4, width: '130px' }, // Status Pembayaran
      ...
      $(row)
        ...
        .on('click keyup', function (this: HTMLElement, e: Event) {
          ...
          if (e.type === 'click' || ke.key === 'Enter' || ke.key === ' ') {
            ...
            (window as any).extRadioShowDetail(rd);
          }
        });
    },
    initComplete: function () {
