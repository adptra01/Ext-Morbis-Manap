...
var __morbis_bg = (() => {
  ...
  function C(a) {
    let r = `[MORBIS Ext] [${a}]`;
    return {
      log: (...t) => console.log(r, ...t),
      warn: (...t) => console.warn(r, ...t),
      error: (...t) => console.error(r, ...t),
    };
  }
  var N = 'http://dev.rsudkotajambi.id/rs',
    G = 'ext-farmasi-app-base';
  var P = ['dev.rsudkotajambi.id', '103.147.236.138', 'localhost', '127.0.0.1'],
    I = '.rsudkotajambi.id';
  function U(a) {
    try {
      let r = new URL(a);
      if (r.protocol !== 'http:' && r.protocol !== 'https:') return !1;
      let t = r.hostname.toLowerCase();
      return P.includes(t) ? !0 : t.endsWith(I);
    } catch {
      return !1;
    }
  }
  function x() {
    try {
      let a = localStorage.getItem(G);
      if (a && U(a)) return a.replace(/\/+$/, '');
    } catch {}
    return N;
  }
  function R(a, r = 'id') {
    return x() + '/api/tts?text=' + encodeURIComponent(a) + '&lang=' + encodeURIComponent(r);
  }
  var E = C('Background'),
    O = 720 * 60 * 1e3,
    A = 'ttsCache:',
  ...
  async function D(a) {
    try {
      let r = L(a),
        n = (await chrome.storage.local.get(r))[r];
      return n ? (Date.now() - n.ts > O ? (await chrome.storage.local.remove(r), null) : n) : null;
    } catch {
      return null;
    }
  }
  async function M(a, r, t) {
    try {
      let n = { mime: r, data: t, ts: Date.now() };
      await chrome.storage.local.set({ [L(a)]: n });
      let e = await chrome.storage.local.get(null),
        s = Object.keys(e).filter((l) => l.startsWith(A));
      if (s.length > w) {
        let l = s
          .map((o) => ({ k: o, ts: e[o].ts }))
          .sort((o, i) => o.ts - i.ts)
          .slice(0, s.length - w);
        await chrome.storage.local.remove(l.map((o) => o.k));
      }
    } catch {}
  }
  async function F() {
    try {
      let a = await chrome.storage.local.get(null),
        r = Date.now(),
        t = Object.entries(a)
          .filter(([n, e]) => n.startsWith(A) && r - e.ts > O)
          .map(([n]) => n);
  ...
  function v(a) {
    if (!a || !a.features) return structuredClone(m);
    let r = Object.keys(m.features),
      t = {};
    for (let e of r)
      a.features[e]
        ? (t[e] = structuredClone(a.features[e]))
        : (t[e] = structuredClone(m.features[e]));
    let n = Object.values(k);
    for (let e of r) {
      let s = m.features[e].allowedRoles,
        l = t[e].allowedRoles;
      !Array.isArray(l) || l.length === 0
        ? (t[e].allowedRoles = [...s])
        : l.filter((i) => !n.includes(i)).length > 0 &&
          (t[e].allowedRoles = l.filter((i) => n.includes(i)));
    }
    for (let e of ['filterPersistence', 'doctorFilterPersistence'])
      t[e] && (t[e].allowedRoles = [...Object.values(k)]);
    if (t.antrianTools) {
      let e = t.antrianTools;
      for (let s of ['admin', 'pendaftaran']) e.allowedRoles.includes(s) || e.allowedRoles.push(s);
    }
    return (
      a.currentRole || (a.currentRole = 'admin'),
      n.includes(a.currentRole) || (a.currentRole = 'admin'),
      (a.features = t),
      a
    );
  }
  async function d() {
    try {
      let a = await chrome.storage.sync.get(u);
      if (!a[u]) {
        let t = structuredClone(m);
        return (await chrome.storage.sync.set({ [u]: t }), t);
      }
      let r = v(structuredClone(a[u]));
      return (await chrome.storage.sync.set({ [u]: r }), r);
    } catch (a) {
      return (E.error('Error loading config:', a), structuredClone(m));
    }
  }
  async function f() {
    try {
      let a = await chrome.storage.sync.get(T);
      if (!a[T]) {
        let n = structuredClone(b);
        return (await chrome.storage.sync.set({ [T]: n }), n);
      }
      let r = a[T],
        t = structuredClone(b);
      return (
        r.forEach((n) => {
          n.isDefault || t.push(n);
        }),
        t.forEach((n) => {
          let e = r.find((s) => s.id === n.id);
          e && n.isDefault && (n.enabled = e.enabled);
        }),
        t
      );
    } catch (a) {
      return (E.error('Error loading URLs:', a), structuredClone(b));
    }
  }
  async function g() {
    let [a, r] = await Promise.all([chrome.tabs.query({}), f()]),
      t = r.filter((n) => n.enabled).map((n) => n.url);
    if (t.length !== 0)
      for (let n of a)
        n.id &&
          n.url &&
          t.some((e) => n.url.startsWith(e)) &&
          chrome.tabs.sendMessage(n.id, { type: 'CONFIG_CHANGED' }).catch(() => {});
  }
  chrome.runtime.onMessage.addListener((a, r, t) => {
    let n = X(a);
    if (!n) return (t({ error: 'Invalid message' }), !1);
    switch ((H(n.type), n.type)) {
      case 'GET_ALL':
        return (
          (async () => {
            let [e, s] = await Promise.all([d(), f()]);
            t({ config: e, urls: s, defaultConfig: m });
          })(),
          !0
        );
      case 'GET_CONFIG':
        return (d().then((e) => t({ config: e })), !0);
      case 'GET_URLS':
        return (f().then((e) => t({ urls: e })), !0);
      case 'SET_ROLE':
        return (
          (async () => {
            let e = await d();
            ((e.currentRole = n.role),
      ...
      case 'TOGGLE_FEATURE':
        return (
          (async () => {
            let e = await d();
            (e.features[n.key] &&
              ((e.features[n.key].enabled = n.enabled),
              await chrome.storage.sync.set({ [u]: e }),
              g()),
              t({ success: !0 }));
          })(),
      ...
      case 'CHANGE_FEATURE_MODE':
        return (
          (async () => {
            let e = await d();
            (e.features[n.key] &&
              ((e.features[n.key].mode = n.mode), await chrome.storage.sync.set({ [u]: e })),
              t({ success: !0 }));
          })(),
          !0
        );
      case 'RESET_CONFIG':
        return (
          (async () => (
            await chrome.storage.sync.set({ [u]: structuredClone(m) }),
            g(),
            t({ success: !0 })
          ))(),
          !0
        ...
        return (
          (async () => {
            ...
            (e.push({
              ...
              await chrome.storage.sync.set({ [T]: e }),
              g(),
              t({ success: !0 }));
          })(),
          !0
      ...
      case 'DELETE_URL':
        return (
          (async () => {
            let e = await f();
            ((e = e.filter((s) => s.id !== n.id || s.isDefault)),
              await chrome.storage.sync.set({ [T]: e }),
              g(),
              t({ success: !0 }));
          })(),
      ...
      case 'TOGGLE_URL':
        return (
          (async () => {
            let e = await f();
            for (let s of e) s.id === n.id && (s.enabled = n.enabled);
            (await chrome.storage.sync.set({ [T]: e }), g(), t({ success: !0 }));
          })(),
          !0
        );
      ...
      case 'PAGE_CONTEXT': {
        ...
        if (e && n.feature) {
          let s = n;
          (y.set(e, { feature: n.feature, data: s.data }),
            E.log('Page context stored for tab', e, ':', n.feature));
        }
      ...
      case 'GET_PAGE_CONTEXT':
        return (
          (async () => {
            let s = (await chrome.tabs.query({ active: !0, currentWindow: !0 }))[0]?.id;
            s && y.has(s) ? t({ context: y.get(s) }) : t({ context: null });
          })(),
          !0
        );
      case 'PROXY_FETCH':
        return (
          (async () => {
            try {
              let { url: e, method: s = 'GET', data: l } = n,
                o = e,
                i = { method: s, credentials: 'include' };
              if (l && typeof l == 'object') {
                let p = new URLSearchParams(l);
                s === 'POST'
                  ? ((i.body = p),
                    (i.headers = { 'Content-Type': 'application/x-www-form-urlencoded' }))
                  : (o += '?' + p.toString());
              }
              let c = await (await fetch(o, i)).text();
              t({ success: !0, html: c });
            } catch (e) {
      ...
      case 'QUEUE_API':
        return (
          (async () => {
            try {
              let { url: e, method: s = 'GET', body: l } = n,
                o = { method: s, cache: 'no-store' };
              l != null &&
                ((o.body = JSON.stringify(l)),
                (o.headers = { 'Content-Type': 'application/json' }));
              let i = await fetch(e, o),
                _ = i.headers.get('content-type') || '',
                c = null;
              try {
                c = await i.json();
              } catch {
              ...
              t({ ok: !0, status: i.status, contentType: _, data: c });
            } catch (e) {
              t({ ok: !1, error: String(e) });
            }
          })(),
          !0
        );
          ...
          (async () => {
            try {
              let { text: e } = n,
                s = await D(e);
              if (s) {
                t({ ok: !0, mime: s.mime, data: s.data });
                return;
              }
              let l = async (i, _ = 5e3) => {
                  let c = await fetch(i, { mode: 'cors', signal: AbortSignal.timeout(_) });
                  if (!c.ok) throw new Error('http ' + c.status);
                  let p = await c.arrayBuffer();
                  if (!p || p.byteLength === 0) throw new Error('empty');
                  return {
                    mime: c.headers.get('content-type') || 'audio/mpeg',
                    data: Array.from(new Uint8Array(p)),
                  };
                },
                o;
              try {
                o = await l('http://127.0.0.1:8765/tts?text=' + encodeURIComponent(e), 3e3);
              } catch {
                let i = await d().catch(() => null);
                if (i && i.features?.ttsServer && i.features.ttsServer.enabled === !1)
                  throw new Error('tts-server-off');
                o = await l(R(e));
              }
              (M(e, o.mime, o.data), t({ ok: !0, mime: o.mime, data: o.data }));
            } catch (e) {
              t({ ok: !1, reason: 'tts-fetch ' + String(e).slice(0, 60) });
            }
          })(),
          !0
        );
      case 'TAB_ACTION':
        return (
          (async () => {
            ...
            try {
              (await chrome.tabs.sendMessage(e, n), t({ success: !0 }));
            } catch (s) {
              (E.log('TAB_ACTION forward failed:', s), t({ success: !1 }));
            }
          })(),
          !0
        );
      case 'TAB_ACTION_RESULT':
        return (chrome.runtime.sendMessage(n).catch(() => {}), t({ success: !0 }), !0);
      default:
        return !1;
    }
  });
  ...
  chrome.runtime.onInstalled.addListener(function () {
    (chrome.alarms.create('morbis-heartbeat', { periodInMinutes: B }),
      ...
      E.log('Heartbeat and state-sync alarms registered'));
  });
  chrome.alarms.onAlarm.addListener(function (a) {
    (a.name === 'morbis-heartbeat' && E.log('Heartbeat: SW alive'),
      a.name === 'morbis-state-sync' && (S().catch(function () {}), F().catch(function () {})));
  });
  async function S() {
    try {
      let a = await d();
      await chrome.storage.session.set({
        ...
        extensionEnabled: a.extensionEnabled,
      });
    } catch (a) {
      E.error('State sync failed:', a);
    }
  }
  async function H(a) {
    [
  ...
  var j = Object.values(h).filter((a) => a !== 'CONFIG_CHANGED');
  function X(a) {
    if (!a || typeof a != 'object') return null;
    let r = a;
    return typeof r.type != 'string' || !j.includes(r.type) ? null : r;
  }
  chrome.action.onClicked.addListener(function (a) {
    a.id && chrome.sidePanel.open({ tabId: a.id }).catch(function () {});
  });
  E.log('Service worker started');
})();
