...
`,
    ...
    `),
          (this.btn = o.querySelector('button')));
      }
      connectedCallback() {
        ((this.btn.disabled = this.hasAttribute('disabled') || this.hasAttribute('loading')),
          ...
          this.btn.addEventListener('click', (o) => {
...
`,
    k = class extends HTMLElement {
      constructor() {
        super();
        let o = m(this);
        o.innerHTML = `<style>${B}</style><slot></slot>`;
      }
    };
...
`,
    ...
    `;
      ...
      connectedCallback() {
        ...
        let o = this.querySelector('[slot="tab"][data-active]');
        o && this.activate(o.getAttribute('data-tab') || '');
      }
      activate(o) {
        o &&
          (this.querySelectorAll('[slot="tab"]').forEach((t) => {
            t.getAttribute('data-tab') === o
              ? t.setAttribute('data-active', '')
              : t.removeAttribute('data-active');
          }),
          this.querySelectorAll('[slot="panel"]').forEach((t) => {
            t.getAttribute('data-panel') === o
              ? t.setAttribute('data-active', '')
              : t.removeAttribute('data-active');
          }),
          this.dispatchEvent(new CustomEvent('ext-tab-change', { detail: { tab: o } })));
      }
    };
...
`,
  ...
  (function () {
    ...
    function S(i) {
      if (!i) return null;
      let s = i.getAttribute('onclick');
      if (!s) return null;
      let r = s.match(/[?&]id=(\d+)/),
        n = s.match(/[?&]id_visit=(\d+)/);
      return r ? { id: r[1], idVisit: n ? n[1] : '' } : null;
    }
    function v(i, s, r = 'sm') {
      let n = document.createElement('ext-btn');
      return (
        n.setAttribute('variant', s),
        n.setAttribute('size', r),
        n.setAttribute('class', e),
        n.setAttribute('style', 'margin-left:5px;'),
        (n.textContent = i),
        n
      );
    }
    function b() {
      document.querySelectorAll('table tbody tr').forEach((i) => {
        if (i.querySelector('.' + e)) return;
        let s = i.querySelector('[onclick*="edit_hasil"],[onclick*="cetak_nota"]');
        if (!s) return;
        let r = s.closest('td');
        if (!r) return;
        let n = S(s);
        if (!n) return;
        let a = n.id,
          c = i.querySelector('td:nth-child(4)')?.textContent?.trim() || '',
          l = v('Batal', 'danger');
        ((l.onclick = () => {
    ...
    function C() {
      document.querySelectorAll('table tbody tr').forEach((i) => {
        if (i.querySelector('.' + e)) return;
        let s = i.querySelector('[onclick*="editBacaan"],[onclick*="showAddFotoRadiologi"]');
        if (!s) return;
        let r = s.closest('td');
        if (!r) return;
        let n = S(s);
    ...
    function L(i, s, r, n) {
      ...
      (d.setAttribute('slot', 'title'), (d.textContent = i));
    ...
    function H() {
      ...
      if (!i) return;
      let s = document.querySelector('.field-group');
      if (!s || s.querySelector('.' + e)) return;
          ...
          L(
            ...
            () => {
              fetch('/routes/radiologi?opsi=batal-radiologi', {
                ...
                body: 'idRadiologi=' + encodeURIComponent(i),
              })
                .then((n) => n.json())
                .then((n) => {
                  if (n.code === 200) {
    ...
    function A() {
      ...
      /\/laboratorium\/input-hasil/.test(i)
        ? b()
        : /\/admisi\/radiologi\/pemeriksaan\/form-edit-bacaan-radiologi/.test(i)
          ? H()
          : /\/admisi\/radiologi\/pemeriksaan/.test(i) && C();
    }
