...
var __morbis_feature = (() => {
  // src/features/autoVerifBilling.ts
  (function() {
    const MAX_WAIT = 100;
    let waited = 0;
    ...
    function overrideSwal() {
      ...
      originalSwal = w.swal;
      w.swal = function swalOverride(...args) {
        if (!isAutoMode && !isBatalMode) {
          return originalSwal.apply(this, args);
        }
        const msgText = args.length === 1 && typeof args[0] === "object" ? (args[0].title || "") + " " + (args[0].text || "") : "";
    ...
    function checkDischargeStatus(callback) {
      const bodyText = document.body?.textContent || "";
      if (bodyText.indexOf("Belum Discharge") === -1 && bodyText.indexOf("belum di-discharge") === -1 && bodyText.indexOf("belum keluar") === -1) {
        callback(false);
        return;
      }
      const parent = getSaveParent();
    ...
    function addAutoVerifButton() {
      if (checkLunasStatus()) return;
      addAutoVerifButtonInternal();
    }
    function addAutoVerifButtonInternal() {
      const parent = getSaveParent();
      if (!parent) return;
      ...
      btn.onclick = function() {
        runAutoVerif(btn, document.getElementById("save"));
      };
    ...
    function fillMultiAsuransi(fields, totalNum) {
      ...
      if (hasExisting) {
        ...
        if (existingSum > 0) {
          let allocated = 0;
          for (let i = 0; i < fields.length; i++) {
            const portion = i < fields.length - 1 ? Math.floor(existingValues[i] / existingSum * totalNum) : totalNum - allocated;
    ...
    function fixRounding(targetTotal) {
      const nomFields = document.querySelectorAll("input.nominalPerAsuransi");
    ...
    function autoFill(totalAll) {
      const klaimBpjs = document.getElementById("klaim_bpjs");
    ...
    function runAutoVerif(btn, saveBtn) {
      const st = btn.parentElement?.querySelector(".ext-avb-status");
      ...
      const noms = document.querySelectorAll("input.nominalPerAsuransi");
      let sum = 0;
    ...
    function runBatalAutoVerif(btn) {
      const st = btn.parentElement?.querySelector(".ext-avb-status");
      if (!st) return;
      btn.setAttribute("disabled", "disabled");
    ...
    function initForm(klaimBpjs, _saveBtn) {
      injectStyle();
      overrideSwal();
      setupSwalObserver();
      if (klaimBpjs.disabled || klaimBpjs.readOnly) {
        ...
        checkDischargeStatus(function(blocked) {
          if (!blocked) {
            addAutoVerifButton();
            checkAutoTrigger();
          }
    ...
    function checkAutoTrigger() {
      ...
      const poll = setInterval(function() {
        const btn = document.querySelector(".ext-avb-primary");
        if (btn) {
          clearInterval(poll);
          setTimeout(function() {
    ...
    function getVisitIdFromRow(row) {
      const links = row.querySelectorAll("a");
      for (let i = 0; i < links.length; i++) {
        const m2 = links[i].getAttribute("href")?.match(/[?&]id_visit=(\d+)/);
        if (m2) return m2[1];
      }
      const els = row.querySelectorAll("[onclick]");
      for (let i = 0; i < els.length; i++) {
        const m2 = els[i].getAttribute("onclick")?.match(/['"]?(\d+)['"]?\s*\)/);
        if (m2) return m2[1];
      }
      const m = row.innerHTML.match(/id_visit[\\?&]*=(\d+)/);
      return m ? m[1] : null;
    }
    function getActionCellParent(row) {
      const allTds = row.querySelectorAll("td");
      let lastWithLinks = null;
    ...
    function addListPageButtons() {
      ...
      for (let t = 0; t < tables.length; t++) {
        ...
        for (let r = 0; r < rows.length; r++) {
          ...
          let autoUrl = "";
          const verifLink = row.querySelector(
...
//# sourceMappingURL=autoVerifBilling.js.map
