...
var __morbis_feature = (() => {
  ...
  var reader = {
    get: (id, fallbackName) => (document.querySelector("#" + id)?.value || (fallbackName ? document.querySelector('input[name="' + fallbackName + '"]')?.value : "") || "").trim(),
    namaPasien: resolveNamaPasien,
    tglLahir: resolveTglLahir
  };
  function getField(id, fallbackName) {
    return document.querySelector("#" + id)?.value?.trim() || (fallbackName ? document.querySelector('input[name="' + fallbackName + '"]')?.value?.trim() : "") || "";
  }
  function escHtml(s) {
  ...
  function renderActionBar(state, code) {
    ...
    const klik = (jenis) => {
      ...
      antrikanResep(idVisit, nomorResep2, jenis, reader).then((code2) => renderActionBar("issued", code2)).catch((e) => alert("[MORBIS Ext] " + (e?.message || "gagal mengantrikan"))).finally(() => {
        if (btn) {
          btn.disabled = false;
