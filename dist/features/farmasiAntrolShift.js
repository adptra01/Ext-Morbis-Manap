...
var __morbis_feature = (() => {
  ...
  function M(e) {
    return new Promise((n) => {
      ...
      let u = o.querySelector('.ext-confirm-body');
      e.message &&
        e.message
          .split(
            `
`,
      ...
      let a = (g) => {
          (o.remove(), document.removeEventListener('keydown', p), n(g));
        },
        p = (g) => {
          g.key === 'Escape' && a(!1);
        };
      ...
      let x = o.querySelector('[data-ext-cancel]');
      (x && x.addEventListener('click', () => a(!1)),
  ...
  function T() {
    ...
    for (let t of n) {
      let o = t.querySelectorAll('td');
      for (let u = 0; u < o.length - 1; u++)
        if (/^tanggal\s*lahir$/i.test(o[u].textContent?.trim() || '') && o[u + 1]) {
          let a = o[u + 1].textContent?.trim();
          if (a && a !== ':') return a;
        }
    }
    return '';
  ...
  (() => {
    ...
    function u() {
      ...
      ((XMLHttpRequest.prototype.open = function (r, d, ...s) {
        return ((this.__extUrl = String(d)), c.apply(this, [r, d, ...s]));
      }),
    ...
    async function p(c, i) {
      try {
        ...
        return (
          ...
          r.find((s) => String(s.ID_PASIEN ?? '') === String(c)) ??
          null
        );
      } catch {
        return null;
    ...
    async function x(c, i, l) {
      ...
      let d = document.querySelector('#id_pasien')?.value ?? '',
        ...
        f = null;
      for (let _ = 0; _ < 5 && !f; _++)
        ((f = await p(d, s)), f || (await new Promise((P) => setTimeout(P, 400))));
      let b = f ? String(f.ID ?? '') : c,
        m = R(),
        y = l === 'racik' ? 'racikan' : 'tunggal',
        I = await A({
          event_id: L('enq', b, c + '-' + y) + '-' + Date.now().toString(36),
          event: 'ENQUEUE',
          resep_id: i,
          nama_pasien: m,
          norm: d || void 0,
          ...
          jenis: y,
          counter: '',
          payload: { idVisit: c, unit: String(f?.NAMA_UNIT ?? ''), waktu: s || '' },
        });
      if (!I.ok) {
    ...
    async function g(c, i) {
      ...
      let r = await A({ event_id: l, event: 'BATAL', queue_number: c, resep_id: i });
      ...
      if (c === 'issued' && i) {
        ((l.innerHTML =
          ...
          l.querySelector('#ext-antrian-batal')?.addEventListener('click', async () => {
            console.log('[MORBIS Ext] BATAL click \u2014 code:', i, '| nomorResep:', r);
            let s = confirm('Batalkan antrian ' + i + '? Resep akan keluar dari daftar panggilan.');
            if ((console.log('[MORBIS Ext] confirm answer:', s), !s)) return;
            let f = l.querySelector('#ext-antrian-batal');
            (f && ((f.disabled = !0), (f.textContent = 'Membatalkan\u2026')), await g(i || '', r));
          }));
        return;
      }
      l.innerHTML =
    ...
    function H() {
      let c = () => {
        ...
        i = () => {
          let r = document.querySelector('#ext-antrian-bar') || c();
          if (!r) return;
          let d = (s) => {
