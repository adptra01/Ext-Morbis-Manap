  ...
  (function() {
    ...
    const PATIENT_INFO_TABS = [
      ...
      },
      {
        ...
        ajax: {
          ...
          method: "GET",
          data: (visit, noRm, _konsulId) => ({ norm: noRm, id_visit: visit })
        }
      }
    ];
    ...
    function enhanceTables() {
      ...
      tables.forEach(function(tbl) {
        ...
        const hasAksiCol = headerTexts.some(function(t) {
          return /aksi/i.test(t);
        });
        ...
        const permintaanIdx = headerTexts.findIndex(function(t) {
          return /permintaan/i.test(t);
        });
        const kesanIdx = headerTexts.findIndex(function(t) {
          return /kesan/i.test(t);
        });
        const anjuranIdx = headerTexts.findIndex(function(t) {
          return /anjuran/i.test(t);
        });
        var actionCol = hasAksiCol ? headerTexts.findIndex(function(t) {
          return /aksi/i.test(t);
        }) : totalCols;
        ...
        rows.forEach(function(row, ri) {
          ...
          var rd = [];
          allCells.forEach(function(td2) {
            rd.push((td2.textContent || "").trim());
          });
    ...
    function openDetail(data) {
      ...
      let fields = '<label>No. RM</label><div class="morbis-cons-fv">' + esc(data.noRm) + " \u2014 " + esc(data.nama) + '</div><label>Unit Asal \u2192 Unit Tujuan</label><div class="morbis-cons-fv">' + esc(data.unitAsal) + " \u2192 " + esc(data.unitTujuan) + '</div><label>Dokter</label><div class="morbis-cons-fv">' + esc(data.dokterMengajukan) + " \u2192 " + esc(data.dokterKonsultasi) + '</div><label>Tanggal Pengajuan</label><div class="morbis-cons-fv">' + esc(data.tanggal) + "</div>";
      if (data.permintaan !== void 0) {
        fields += '<label>Permintaan Konsultasi</label><div class="morbis-cons-fv">' + esc(data.permintaan) + "</div>";
      }
      if (data.kesan !== void 0) {
        fields += '<label>Kesan</label><div class="morbis-cons-fv">' + esc(data.kesan) + "</div>";
      }
      if (data.anjuran !== void 0) {
      ...
      ov.innerHTML = '<div class="morbis-cons-content"><div class="morbis-cons-header"><h2>Detail Konsultasi \u2014 ' + esc(data.nama || "") + '</h2><button class="morbis-cons-close">&times;</button></div><div class="morbis-cons-body">' + fields + "</div></div>";
      document.body.appendChild(ov);
      const closeBtn = ov.querySelector(".morbis-cons-close");
      if (closeBtn)
    ...
    function openTabs(data) {
      ...
      PATIENT_INFO_TABS.forEach(function(tab, i) {
        var active = i === 0;
        tabsHtml += '<button class="morbis-tab-btn' + (active ? " morbis-tab-active" : "") + '" data-tab="' + tab.id + '">' + esc(tab.label) + "</button>";
        panelsHtml += '<div class="morbis-tab-panel' + (active ? " morbis-tab-active" : "") + '" data-panel="' + tab.id + '"><div class="morbis-tab-body" style="padding:20px;min-height:400px;"><div style="text-align:center;padding:40px;color:#999;">Memuat...</div></div></div>';
      });
      var ov = document.createElement("div");
      ov.className = "morbis-cons-overlay";
      ov.id = "morbis-cons-tabs";
      ov.innerHTML = '<div class="morbis-cons-content" style="width:95%;max-width:1200px;"><div class="morbis-cons-header"><h2>' + esc(data.nama || "") + " (" + esc(data.noRm || "") + ')</h2><button class="morbis-cons-close">&times;</button></div><div class="morbis-cons-tab-bar">' + tabsHtml + '</div><div class="morbis-cons-body" style="padding:0;">' + panelsHtml + "</div></div>";
      document.body.appendChild(ov);
      injectPageScripts();
      var closeBtn = ov.querySelector(".morbis-cons-close");
      if (closeBtn)
        closeBtn.onclick = function() {
          ov.remove();
        };
      ...
      ov.querySelectorAll(".morbis-tab-btn").forEach(function(btn) {
        btn.addEventListener("click", function() {
          ...
          if (tabId && !loadedTabs[tabId]) {
            loadTabContent(tabId, panel, data);
      ...
      if (firstTab) {
        ...
        var firstPanel = ov.querySelector('.morbis-tab-panel[data-panel="' + firstId + '"]');
    ...
    function loadTabContent(tabId, panel, data) {
      var tab = PATIENT_INFO_TABS.find(function(t) {
        return t.id === tabId;
      });
      if (!tab) return;
    ...
    function injectPenunjangFix(visit, noRm) {
...
//# sourceMappingURL=consultationEnhancer.js.map
