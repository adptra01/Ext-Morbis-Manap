...
`,
  ...
  function L(e) {
    return new Promise((t, n) => {
      chrome.runtime.sendMessage(e, (o) => {
        chrome.runtime.lastError ? n(chrome.runtime.lastError) : t(o);
      });
    });
  }
  var F = 'http://dev.rsudkotajambi.id/rs',
    z = null,
    m = null;
  async function $() {
    try {
  ...
  var Q = ['http://dev.rsudkotajambi.id/rs', 'http://103.147.236.138/rs'],
    ...
    K = ['.rsudkotajambi.id', '.ddev.site'];
  function W(e) {
    try {
      let t = new URL(e);
      if (t.protocol !== 'http:' && t.protocol !== 'https:') return !1;
      let n = t.hostname.toLowerCase();
      return X.includes(n) ? !0 : K.some((o) => n.endsWith(o));
    } catch {
      return !1;
    }
  }
  async function J(e, t, n) {
    return L({ type: 'QUEUE_API', url: e, method: t, body: n });
  }
  function Y(e, t) {
    return new Promise((n, o) => {
      let r = setTimeout(() => o(new Error('timeout')), t);
      e.then((a) => {
        (clearTimeout(r), n(a));
      }).catch((a) => {
        (clearTimeout(r), o(a));
    ...
    return (
      ...
      ((m = (async () => {
        try {
          ...
          if (n && W(n)) return n.replace(/\/+$/, '');
        } catch {}
        let e = await $(),
          t = [...new Set([...e, ...Q])];
        for (let n of t)
          try {
            let o = await Y(J(n + '/api/queue/lookup?resep_id=probe', 'GET'), 2500),
              r = o.contentType || '';
  ...
  async function O(e) {
    try {
      ...
      let n = t.message;
      return (
        n !== C && (console.warn('[MORBIS Ext] queue sync gagal:', n), (C = n)),
        await Z(e),
        { ok: !1 }
      );
    }
    ...
    let n = document.documentElement,
      ...
      r = window.setInterval(() => {
        n.getAttribute('data-ext-antrian-farmasi') === '1'
          ...
          : Date.now() - o > t && (window.clearInterval(r), oe());
      }, 200);
  }
  function oe() {
  ...
  var re = '/v2/antrol/search',
    ie = 'sub=update_v2',
    ae = '/public/antrian-farmasi-v2/list-antrian-v2',
    w = null,
    y = null;
  ...
  function M(e, t) {
    return (String(e.NAMA_PAS ?? e.NAMA_PASIEN ?? '').trim() || ue(t)).toUpperCase();
  }
  async function pe(e) {
    try {
      let t = await U(e),
      ...
      let l = M(t, r),
        b = (async () => {
          for (let g = 0; g < 3; g++) {
            try {
              let d = await le(),
                v =
                  ce(d, String(t.ID_PASIEN ?? ''), String(t.WAKTU_PENGAJUAN ?? '')) ??
                  d.find((q) => String(q.ID ?? '') === n);
              if (v) return v;
            } catch {}
        ...
        D = O({
          event_id: N('enq', n, s) + '-' + Date.now().toString(36),
          event: 'ENQUEUE',
          resep_id: e,
          nama_pasien: l,
        ...
        [h, T] = await Promise.all([b, D]);
      T.ok || u('ENQUEUE app gagal (app tidak terjangkau?) \u2014 antrian tetap jalan di MORBIS');
      let p = T.queue_number || s;
      u('nomor publik', p);
      let H = h?.SHIFT || (i ? de(i) : '') || '';
      if (i && !i.hasAttribute('data-ext-code')) {
        let g = i.querySelector('button'),
          d = g ? g.outerHTML : '';
        ((i.innerHTML = `${p}<br>Shift : ${H || '-'}` + (d ? '<br>' + d : '')),
  ...
  function P() {
    try {
      document.querySelectorAll('tr[id]').forEach((e) => {
        ...
        let o = t.getAttribute('data-ext-code') || G(t),
          r = e.getAttribute('id') || '';
        !o ||
          !r ||
          (t.setAttribute('data-ext-code', o), t.setAttribute('data-ext-resep', r), B(t, o, r));
      });
    } catch {}
  }
  R(() => {
    (P(), (y = window.setInterval(P, 4e3)));
  });
  window.addEventListener('beforeunload', () => {
    (w !== null && clearInterval(w), y !== null && clearInterval(y));
  });
})();
