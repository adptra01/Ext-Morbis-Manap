...
var __morbis_feature = (() => {
  ...
  async function handleNoAntrian(idResep) {
    try {
      ...
      const rowPromise = (async () => {
        for (let i = 0; i < 3; i++) {
          try {
            ...
            const found = findRow(rows, String(data.ID_PASIEN ?? ""), String(data.WAKTU_PENGAJUAN ?? "")) ?? rows.find((r) => String(r.ID ?? "") === idVisit);
      ...
      if (antrianCell && !antrianCell.hasAttribute("data-ext-code")) {
        ...
        antrianCell.innerHTML = `${esc(publicNumber)}<br>Shift : ${esc(shift || "-")}` + (btnHtml ? "<br>" + btnHtml : "");
  ...
  function markCetakUlang(cell, code, idResep) {
    ...
    klon.style.cssText = "margin-top:4px;padding:3px 8px;font-size:11px;border:1px solid #0d6efd;background:#e7f1ff;color:#0d6efd;border-radius:6px;cursor:pointer;";
