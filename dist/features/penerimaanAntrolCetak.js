    ...
    try {
      ...
      for (let i = 0; i < 5 && !row; i++) {
        try {
          ...
          row = findRow(rows, String(data.ID_PASIEN ?? ""), String(data.WAKTU_PENGAJUAN ?? "")) ?? rows.find((r) => String(r.ID ?? "") === idVisit);
        } catch {
        }
      ...
      if (antrianCell && !antrianCell.hasAttribute("data-ext-code")) {
        ...
        antrianCell.innerHTML = `${publicNumber}<br>Shift : ${shift || "-"}` + (btnHtml ? "<br>" + btnHtml : "");
