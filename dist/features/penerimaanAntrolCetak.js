"use strict";var __morbis_feature=(()=>{var _="ext-batch-shared-style";function F(){if(document.getElementById(_))return;let e=document.createElement("style");e.id=_,e.textContent=`
    .ext-modal-content {
...
`).forEach((l,b)=>{b>0&&r.appendChild(document.createElement("br")),r.appendChild(document.createTextNode(l))});let a=l=>{o.remove(),document.removeEventListener("keydown",i),t(l)},i=l=>{l.key==="Escape"&&a(!1)};o.querySelector(".ext-modal-close").addEventListener("click",()=>a(!1)),o.addEventListener("click",l=>{l.target===o&&a(!1)}),o.querySelector("[data-ext-ok]").addEventListener("click",()=>a(!0));let s=o.querySelector("[data-ext-cancel]");s&&s.addEventListener("click",()=>a(!1)),document.addEventListener("keydown",i),document.body.appendChild(o)})}var k="RSUD H. Abdul Manap";function E(e){let t=window.open("","_blank","width=400,height=560");if(!t)return A({title:"Popup Diblokir",message:"Izinkan popup untuk mencetak.",variant:"warning",okLabel:"OK",hideCancel:!0}),!1;let n=e.jenis||e.unit?`<div style="font-size:16px;margin-top:2px;">${[e.jenis,e.unit].filter(Boolean).join(" \xB7 ")}</div>`:"",o=e.tglLahir?`<div style="font-size:13px;margin-top:4px;color:#555;">${e.tglLahir}</div>`:"";return t.document.write('<html><head><title>Antrian Farmasi</title></head><body style="width:320px;padding-top:10px;font-family:Arial,Helvetica,sans-serif;text-align:center;"><div style="font-size:16px;font-weight:bold;text-transform:uppercase;">'+k+`</div><div style="font-size:14px;margin-top:2px;">Antrian Farmasi</div><div style="margin-top:14px;"><div style="font-size:110px;font-weight:900;letter-spacing:-2px;line-height:1;">${e.code}</div></div><div style="font-size:20px;font-weight:bold;margin-top:10px;">${e.nama}</div>`+o+n+`<div style="font-size:11px;margin-top:10px;color:#333;">${e.tanggal}</div><div style="font-size:13px;margin-top:14px;color:#555;">Silakan menunggu panggilan</div></body></html>`),t.document.close(),window.setTimeout(()=>{try{t.focus(),t.print()}catch{}},300),!0}function L(e){return new Promise((t,n)=>{chrome.runtime.sendMessage(e,o=>{chrome.runtime.lastError?n(chrome.runtime.lastError):t(o)})})}var z="http://dev.rsudkotajambi.id/rs",$=null,m=null;async function Q(){try{return((await chrome.storage.sync.get("extensionCustomUrls")).extensionCustomUrls??[]).filter(n=>n.url&&n.enabled!==!1).map(n=>n.url.replace(/\/+$/,"")+"/rs")}catch{return[]}}var X=["http://dev.rsudkotajambi.id/rs","http://103.147.236.138/rs"],K=["dev.rsudkotajambi.id","103.147.236.138","localhost","127.0.0.1"],W=[".rsudkotajambi.id",".ddev.site"];function J(e){try{let t=new URL(e);if(t.protocol!=="http:"&&t.protocol!=="https:")return!1;let n=t.hostname.toLowerCase();return K.includes(n)?!0:W.some(o=>n.endsWith(o))}catch{return!1}}async function Y(e,t,n){return L({type:"QUEUE_API",url:e,method:t,body:n})}function V(e,t){return new Promise((n,o)=>{let r=setTimeout(()=>o(new Error("timeout")),t);e.then(a=>{clearTimeout(r),n(a)}).catch(a=>{clearTimeout(r),o(a)})})}function I(){return m||(m=(async()=>{try{let n=localStorage.getItem("ext-farmasi-app-base");if(n&&J(n))return n.replace(/\/+$/,"")}catch{}let e=await Q(),t=[...new Set([...e,...X])];for(let n of t)try{let o=await V(Y(n+"/api/queue/lookup?resep_id=probe","GET"),2500),r=o.contentType||"";if((o.status===200||o.status===422)&&r.includes("application/json"))return $=n,n}catch{}return z})(),m)}var C="";async function O(e){try{let t={...e};if(e.event==="ENQUEUE"&&delete t.queue_number,e.event==="BATAL"&&!e.queue_number)return console.warn("[MORBIS Ext] BATAL tanpa queue_number \u2014 dilewati"),{ok:!1};let n=await I(),o=new AbortController,r=setTimeout(()=>o.abort(),8e3),a=await fetch(n+"/api/queue/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t),cache:"no-store",credentials:"omit",signal:o.signal});if(clearTimeout(r),!a.ok){let s="";try{s=(await a.json())?.message||""}catch{}throw new Error("HTTP "+a.status+(s?" \u2014 "+s:""))}let i=await a.json();return{ok:!!i.ok,queue_number:i.queue?.queue_number,created:i.created,duplicate:i.duplicate}}catch(t){let n=t.message;return n!==C&&(console.warn("[MORBIS Ext] queue sync gagal:",n),C=n),await ee(e),{ok:!1}}}var c="ext-queue-retry-queue",Z=20;async function ee(e){try{let t=(await chrome.storage.local.get(c))[c]??[];if(t.some(n=>n.event_id===e.event_id))return;t.push(e),t.length>Z&&t.shift(),await chrome.storage.local.set({[c]:t}),console.log("[MORBIS Ext] disimpan ke retry queue:",e.event,e.queue_number??"")}catch{}}async function te(){try{return(await chrome.storage.local.get(c))[c]??[]}catch{return[]}}async function S(e){try{let n=((await chrome.storage.local.get(c))[c]??[]).filter(o=>o.event_id!==e);await chrome.storage.local.set({[c]:n})}catch{}}async function ne(){let e=await te();if(e.length)for(let t of[...e])try{(await oe(t)).ok&&(await S(t.event_id),console.log("[MORBIS Ext] retry queue sukses:",t.event,t.queue_number??""))}catch(n){let o=n.message??"";(o.includes("HTTP 404")||o.includes("HTTP 422"))&&(await S(t.event_id),console.log("[MORBIS Ext] retry queue buang (stale):",t.event,t.queue_number??"",o))}}async function oe(e){let t={...e};e.event==="ENQUEUE"&&delete t.queue_number;let n=await I(),o=new AbortController,r=setTimeout(()=>o.abort(),8e3),a=await fetch(n+"/api/queue/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t),cache:"no-store",credentials:"omit",signal:o.signal});if(clearTimeout(r),!a.ok)throw new Error("HTTP "+a.status);let i=await a.json();return{ok:!!i.ok,queue_number:i.queue?.queue_number}}setInterval(()=>{ne()},1e4);function N(e,t,n){return`${e}-${t}-${n}-${new Date().toISOString().slice(0,10)}`}function R(e,t=5e3){let n=document.documentElement,o=Date.now(),r=window.setInterval(()=>{n.getAttribute("data-ext-antrian-farmasi")==="1"?(window.clearInterval(r),e()):Date.now()-o>t&&(window.clearInterval(r),re())},200)}function re(){if(!document.body||document.getElementById("ext-feature-gate-notif"))return;let e=document.createElement("div");e.id="ext-feature-gate-notif",e.textContent="\u26A0\uFE0F Fitur antrian tidak aktif \u2014 muat ulang halaman (F5)",e.style.cssText="position:fixed;top:8px;right:8px;z-index:999999;background:#dc3545;color:#fff;padding:8px 16px;border-radius:6px;font:13px system-ui,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.2);",document.body.appendChild(e),setTimeout(()=>e.remove(),1e4)}var ie="/v2/antrol/search",ae="sub=update_v2",se="/public/antrian-farmasi-v2/list-antrian-v2",w=null,y=null;if(window.__extPenerimaanAntrol)throw new Error("skip double inject penerimaanAntrolCetak");window.__extPenerimaanAntrol=!0;function u(...e){console.log("[MORBIS Ext] penerimaanAntrolCetak:",...e)}function P(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function G(e){let t=await fetch(`/inventory/resep/akses/penerimaan?type=ajax&opsi=data-resep-new&q=1&id=${encodeURIComponent(e)}`,{credentials:"include",cache:"no-store"});if(!t.ok)throw new Error("data-resep-new HTTP "+t.status);return await t.json()}async function le(e){return(await fetch(`${ie}?${ae}`,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`id=${encodeURIComponent(e)}&taskid=6`,credentials:"include"})).ok}async function ce(){let e=await fetch(se,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:"type=check_antrian",cache:"no-store",credentials:"include"});if(!e.ok)throw new Error("check_antrian HTTP "+e.status);let t=await e.json();if(!Array.isArray(t))throw new Error("bukan array");return t}function de(e,t,n){let o=String(n??"");return e.find(r=>String(r.ID_PASIEN??"")!==String(t)?!1:o?String(r.WAKTU??"").slice(0,16)===o.slice(0,16):!0)}function ue(e){if(!e)return"";let t=(e.textContent||"").match(/Shift\s*:\s*([A-Za-z0-9]+)/i);return t?t[1]:""}function M(e){if(!e)return"";let t=(e.textContent||"").match(/\b[A-Z]{2,3}-\d+\b/);return t?t[0]:""}function pe(e){if(!e)return"";let t=e.querySelectorAll("td"),n="";for(let o of Array.from(t).slice(3,5)){let r=(o.textContent||"").trim();r.length>n.length&&!/^[0-9\s.:-]+$/.test(r)&&(n=r)}return n}function D(e,t){return(String(e.NAMA_PAS??e.NAMA_PASIEN??"").trim()||pe(t)).toUpperCase()}async function ge(e){try{let t=await G(e),n=String(t.ID_VISIT??"");if(!n)throw new Error("ID_VISIT kosong");u("antrikan idVisit="+n,"resep",e);let o=await le(n);u("antrol",o?"OK":"gagal");let r=document.querySelector(`tr[id="${e}"]`),i=(r?Array.from(r.querySelectorAll("td")):[])[2],s=M(i)||"";if(!s){u("nomor native belum ada utk",n),alert("Nomor antrian belum terbit. Coba lagi.");return}u("nomor publik",s);let l=D(t,r),b=(async()=>{for(let g=0;g<3;g++){try{let d=await ce(),v=de(d,String(t.ID_PASIEN??""),String(t.WAKTU_PENGAJUAN??""))??d.find(j=>String(j.ID??"")===n);if(v)return v}catch{}await new Promise(d=>setTimeout(d,200))}})(),H=O({event_id:N("enq",n,s)+"-"+Date.now().toString(36),event:"ENQUEUE",resep_id:e,nama_pasien:l,norm:String(t.ID_PASIEN??""),shift:"",jenis:"",counter:"",payload:{idVisit:n,unit:String(t.UNIT_TUJUAN_DEPO??""),waktu:String(t.WAKTU_PENGAJUAN??"")}}),[h,T]=await Promise.all([b,H]);T.ok||u("ENQUEUE app gagal (app tidak terjangkau?) \u2014 antrian tetap jalan di MORBIS");let p=T.queue_number||s;u("nomor publik",p);let q=h?.SHIFT||(i?ue(i):"")||"";if(i&&!i.hasAttribute("data-ext-code")){let g=i.querySelector("button"),d=g?g.outerHTML:"";i.innerHTML=`${P(p)}<br>Shift : ${P(q||"-")}`+(d?"<br>"+d:""),i.setAttribute("data-ext-code",p),i.setAttribute("data-ext-resep",e),B(i,p,e)}E({nomorResep:e,nama:l,jenis:h?.JENIS??"",unit:String(h?.NAMA_UNIT??t.UNIT_TUJUAN_DEPO??""),tanggal:String(t.WAKTU_PENGAJUAN??"").slice(0,10),code:p})}catch(t){u("gagal",t),alert("[MORBIS Ext] Gagal mengantrikan resep: "+String(t.message??t))}}function B(e,t,n){let o=e.querySelector("button");if(!o)return;let r=o.cloneNode(!0);r.textContent="\u{1F5A8} Cetak Kembali",r.title=t+" \u2014 cetak ulang kartu tanpa mengantrikan lagi",r.style.cssText="margin-top:4px;padding:3px 8px;font-size:11px;border:1px solid #0d6efd;background:#e7f1ff;color:#0d6efd;border-radius:6px;cursor:pointer;",r.addEventListener("click",a=>{a.preventDefault(),a.stopPropagation(),(async()=>{try{let i=await G(n);E({nomorResep:n,nama:D(i,e.closest("tr")),jenis:"",unit:String(i.UNIT_TUJUAN_DEPO??""),tanggal:String(i.WAKTU_PENGAJUAN??"").slice(0,10),code:t})}catch(i){alert("[MORBIS Ext] Gagal cetak ulang: "+String(i.message??i))}})()}),o.replaceWith(r)}function x(){let e=window;if(!e.no_antrian||e.no_antrian.__ext)return;let t=e.no_antrian,n=o=>{ge(String(o))};n.__ext=!0,e.no_antrian=n}function f(){try{document.querySelectorAll("table").forEach(e=>{let t=Array.from(e.querySelectorAll("th")),n=-1;t.forEach((o,r)=>{/no\.?\s*antrian|nomor\s*antrian/i.test((o.textContent||"").trim())&&(n=r)}),!(n<0)&&(t.forEach((o,r)=>{r===n&&(o.style.display="none")}),e.querySelectorAll("tr").forEach(o=>{let r=o.children[n];r&&(r.style.display="none")}))})}catch{}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{x(),f()},{once:!0}):(x(),f());window.setTimeout(x,1e3);window.setTimeout(x,3e3);window.setTimeout(f,1e3);window.setTimeout(f,3e3);w=window.setInterval(f,3e3);function U(){try{document.querySelectorAll("tr[id]").forEach(e=>{let t=e.children[2];if(!t)return;let n=t.querySelector("button");if(!n||n.textContent?.includes("Cetak"))return;let o=t.getAttribute("data-ext-code")||M(t),r=e.getAttribute("id")||"";!o||!r||(t.setAttribute("data-ext-code",o),t.setAttribute("data-ext-resep",r),B(t,o,r))})}catch{}}R(()=>{U(),y=window.setInterval(U,4e3)});window.addEventListener("beforeunload",()=>{w!==null&&clearInterval(w),y!==null&&clearInterval(y)});})();
