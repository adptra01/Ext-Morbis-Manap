...
var __morbis_feature = (() => {
  ...
  function fetchWithTimeout(url, init2 = {}, timeoutMs = 3e4) {
    const ac = new AbortController();
    const timer = setTimeout(() => ac.abort(), timeoutMs);
    return fetch(url, { ...init2, signal: ac.signal }).finally(() => clearTimeout(timer));
  }
  ...
  async function processExport(url) {
    ...
    try {
      const res = await fetchWithTimeout(url, { credentials: 'include', cache: 'no-store' }, 3e4);
