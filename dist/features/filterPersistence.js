...
var __morbis_feature = (() => {
  ...
  function l() {
    ...
    for (let t of Object.keys(a)) {
      let n = a[t];
      if (!(e !== n.pattern && e !== n.pattern + '/'))
        return !i.currentConfig?.features?.[t]?.enabled || !i.ExtensionCore.isFeatureAllowed(t)
          ? null
          : n;
    }
    return null;
  ...
  function f() {
    ...
    (e.fields.forEach(function (n) {
      ...
      i.CookieFilterStorage.set(e.storageKey, t),
      console.log('Filter saved:', e.storageKey, t));
  }
  function m() {
    let e = l();
    if (!e) return;
    let t = i.CookieFilterStorage.get(e.storageKey);
    t &&
      (e.fields.forEach(function (n) {
        let r = document.getElementById(n);
        r &&
          ...
          r.dispatchEvent(new KeyboardEvent('keyup', { bubbles: !0 })),
          (n === 'awal' || n === 'akhir' || n === 'tanggalAwal' || n === 'tanggalAkhir') &&
            setTimeout(function () {
              r.dispatchEvent(new Event('blur', { bubbles: !0 }));
            }, 50));
      }),
      console.log('Filter restored:', e.storageKey, t));
  }
  function b() {
    let e = l();
    e &&
      (i.CookieFilterStorage.remove(e.storageKey),
      e.fields.forEach(function (t) {
        let n = document.getElementById(t);
        n && (n.value = '');
      }),
      console.log('Filter cleared:', e.storageKey));
  }
  function p(e) {
    if (!e.scopeField) return document;
    let t = document.getElementById(e.scopeField);
    return (t && (t.closest('form') || t.closest('table') || t.parentElement)) || document;
  }
  function g() {
    let e = l();
    if (!e) return;
    let t = p(e);
    for (let n of e.cariButtonSelectors) {
      let r = t.querySelectorAll(n);
      for (let s of Array.from(r)) {
        let o = s.tagName === 'I' ? s.closest('button') : s;
        o &&
          !o.dataset.filterBound &&
          ((o.dataset.filterBound = 'true'), o.addEventListener('click', f));
      }
    }
    for (let n of e.batalButtonSelectors) {
      let r = t.querySelectorAll(n);
      for (let s of Array.from(r)) {
        let o = s.tagName === 'I' ? s.closest('button') : s;
        o &&
          !o.dataset.filterBound &&
          ((o.dataset.filterBound = 'true'), o.addEventListener('click', b));
      }
    }
  }
  function F() {
    ...
    (t && i.CookieFilterStorage.migrateFromLocalStorage(t, e.storageKey),
      console.log('Running Filter Persistence:', e.storageKey),
