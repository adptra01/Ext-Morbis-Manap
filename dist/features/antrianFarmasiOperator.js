...
var __morbis_feature = (() => {
  ...
  function showFeatureGateNotif() {
    ...
    banner.style.cssText = "position:fixed;top:8px;right:8px;z-index:999999;background:#dc3545;color:#fff;padding:8px 16px;border-radius:6px;font:13px system-ui,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.2);";
  ...
  function printKartuAntrian(data) {
    ...
    const tglLahirLine = data.tglLahir ? `<div style="font-size:13px;margin-top:4px;color:#555;">${data.tglLahir}</div>` : "";
    win.document.write(
      '<html><head><title>Antrian Farmasi</title></head><body style="width:320px;padding-top:10px;font-family:Arial,Helvetica,sans-serif;text-align:center;"><div style="font-size:16px;font-weight:bold;text-transform:uppercase;">' + HOSPITAL_NAME + `</div><div style="font-size:14px;margin-top:2px;">Antrian Farmasi</div><div style="margin-top:14px;"><div style="font-size:110px;font-weight:900;letter-spacing:-2px;line-height:1;">${data.code}</div></div><div style="font-size:20px;font-weight:bold;margin-top:10px;">${data.nama}</div>` + tglLahirLine + jenisLine + `<div style="font-size:11px;margin-top:10px;color:#333;">${data.tanggal}</div><div style="font-size:13px;margin-top:14px;color:#555;">Silakan menunggu panggilan</div></body></html>`
    );
    win.document.close();
    window.setTimeout(() => {
      try {
  ...
  var ICONS = {
    speaker: '<path d="M11 5 6 9H2v6h4l5 4V5z"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/>',
    ...
    volume: '<path d="M11 5 6 9H2v6h4l5 4V5Z"/><path d="M15.5 8.5a5 5 0 0 1 0 7"/><path d="M18.4 5.6a9 9 0 0 1 0 12.8"/>'
  };
  function svg(name, size = 16, color = "#212529") {
    return '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="' + color + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="display:inline-block;visibility:visible;vertical-align:middle;flex:none;">' + (ICONS[name] || "") + "</svg>";
  }
  var STATUS_META = {
  ...
  var lastTanggal = "";
  function catOf(num) {
    return String(num || "").toUpperCase().startsWith("R") ? "racikan" : "tunggal";
  }
  ...
  function printSheetA4() {
    ...
    const rows = [...lastRows].sort(
      (a, b) => a.queue_number.localeCompare(b.queue_number, void 0, { numeric: true })
    );
    const cards = rows.map((r) => {
      const jenisLabel = r.jenis === "racikan" ? "Racikan" : r.jenis === "tunggal" ? "Non Racikan" : "";
      const jenisLine = jenisLabel ? `<div style="font-size:16px;margin-top:2px;">${jenisLabel}</div>` : "";
      return '<div style="width:320px;padding-top:10px;padding-bottom:4px;font-family:Arial,Helvetica,sans-serif;text-align:center;page-break-after:always;"><div style="font-size:16px;font-weight:bold;text-transform:uppercase;">' + HOSPITAL_NAME + '</div><div style="font-size:14px;margin-top:2px;">Antrian Farmasi</div><div style="font-size:13px;margin-top:4px;color:#555;">' + lastTanggal + `</div><div style="margin-top:14px;"><div style="font-size:110px;font-weight:900;letter-spacing:-2px;line-height:1;">${r.queue_number}</div></div><div style="font-size:20px;font-weight:bold;margin-top:10px;">${r.nama_pasien || "-"}</div>` + jenisLine + '<div style="font-size:13px;margin-top:14px;color:#555;">Silakan menunggu panggilan</div></div>';
    }).join("");
    win.document.write(
      '<html><head><title>Antrian Farmasi \u2014 Kartu</title><style>@media print{@page{margin:0}}</style></head><body style="margin:0;font-family:Arial,Helvetica,sans-serif;">' + cards + "</body></html>"
    );
    win.document.close();
    window.setTimeout(() => {
      try {
  ...
  function openDialog(title, bodyHtml, onOk, okLabel = "Simpan") {
    const overlay = document.createElement("div");
    overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:2147483000;display:flex;align-items:center;justify-content:center;";
    overlay.innerHTML = '<div style="background:#fff;border-radius:14px;padding:18px;width:400px;max-width:94vw;box-shadow:0 10px 40px rgba(0,0,0,.25);font:14px/1.5 system-ui,sans-serif;color:#212529;"><div style="font-size:15px;font-weight:800;margin-bottom:12px;">' + title + '</div><div class="ext-op-dlg-body"></div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:14px;"><button class="ext-op-dlg-cancel" style="padding:8px 14px;border:1px solid #ced4da;background:#fff;border-radius:8px;cursor:pointer;">Batal</button><button class="ext-op-dlg-ok" style="padding:8px 14px;border:none;background:#2193cf;color:#fff;border-radius:8px;cursor:pointer;font-weight:700;">' + okLabel + "</button></div></div>";
    document.body.appendChild(overlay);
  ...
  function openSetCounterDialog() {
    ...
    const body = '<div style="margin-bottom:10px;font-size:13px;color:#495057;">Penomoran terlewat / kendala? Set nomor terakhir yang sudah terbit per jenis \u2014 antrian berikutnya lanjut dari nomor itu. Isi keduanya lalu Simpan.</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;"><div style="background:#f8f9fa;border:1px solid #dee2e6;border-radius:10px;padding:10px;"><div style="font-weight:800;color:#2193cf;margin-bottom:6px;">T \u2014 Non Racikan</div><input id="ext-op-cnt-seq-t" type="number" min="0" max="9999" value="' + lastT + '" style="width:100%;padding:7px 10px;border:1px solid #ced4da;border-radius:8px;font-size:15px;box-sizing:border-box;"><div style="margin-top:6px;font-size:11px;color:#6c757d;">Terakhir: ' + lastT + '</div></div><div style="background:#f8f9fa;border:1px solid #dee2e6;border-radius:10px;padding:10px;"><div style="font-weight:800;color:#d97706;margin-bottom:6px;">R \u2014 Racikan</div><input id="ext-op-cnt-seq-r" type="number" min="0" max="9999" value="' + lastR + '" style="width:100%;padding:7px 10px;border:1px solid #ced4da;border-radius:8px;font-size:15px;box-sizing:border-box;"><div style="margin-top:6px;font-size:11px;color:#6c757d;">Terakhir: ' + lastR + "</div></div></div>";
    openDialog("Set Nomor Lanjutan", body, (root) => {
      const tVal = Math.max(
        0,
        parseInt(root.querySelector("#ext-op-cnt-seq-t").value, 10) || 0
      );
      const rVal = Math.max(
        0,
        parseInt(root.querySelector("#ext-op-cnt-seq-r").value, 10) || 0
      );
      void Promise.all([postSetCounter("T", tVal), postSetCounter("R", rVal)]).then(
        () => {
          ...
          alert(
            "Nomor lanjutan tersimpan.\n\nT (Non Racikan) \u2192 lanjut T-" + String(tVal + 1).padStart(2, "0") + "\nR (Racikan) \u2192 lanjut R-" + String(rVal + 1).padStart(2, "0")
          );
          void render();
        },
  ...
  function printBlankSheet(prefix, from, to) {
    ...
    for (let i = from; i <= to; i++) {
      cards.push(
        '<div style="width:320px;padding-top:10px;padding-bottom:4px;font-family:Arial,Helvetica,sans-serif;text-align:center;page-break-after:always;"><div style="font-size:16px;font-weight:bold;text-transform:uppercase;">' + HOSPITAL_NAME + '</div><div style="font-size:14px;margin-top:2px;">Antrian Farmasi</div><div style="font-size:13px;margin-top:4px;color:#555;">' + (prefix === "R" ? "Racikan (R)" : "Non Racikan (T)") + " \u2014 " + lastTanggal + `</div><div style="margin-top:14px;"><div style="font-size:110px;font-weight:900;letter-spacing:-2px;line-height:1;">${prefix}-${String(i).padStart(2, "0")}</div></div><div style="font-size:13px;margin-top:14px;color:#555;">Silakan menunggu panggilan</div></div>`
      );
    }
    win.document.write(
      '<html><head><title>Antrian Farmasi \u2014 Sheet A4</title><style>@media print{@page{margin:0}}</style></head><body style="margin:0;font-family:Arial,Helvetica,sans-serif;">' + cards.join("") + "</body></html>"
    );
    win.document.close();
    window.setTimeout(() => {
      try {
  ...
  function openPrintSheetDialog() {
    ...
    openDialog("Cetak Banyak Antrian", body, (root) => {
      ...
      const from = Math.max(
        1,
        parseInt(root.querySelector("#ext-op-sheet-from").value, 10) || 1
      );
      const to = Math.max(
        from,
        Math.min(
          9999,
          parseInt(root.querySelector("#ext-op-sheet-to").value, 10) || from
        )
      );
      if (to - from + 1 > 300) {
    ...
    document.querySelectorAll('input[name="ext-op-sheet-src"]').forEach(
  ...
  function iconBtn(ev, icon, title, num, eventId, opts) {
    return '<button class="ext-op-act" data-ev="' + ev + '" data-num="' + num + '" data-eid="' + eventId + '" data-tip="' + title + '" title="' + title + '" aria-label="' + title + '" style="width:34px;height:34px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #ced4da;background:#fff;color:' + (opts?.danger ? "#b02a37" : "#212529") + ';border-radius:8px;cursor:pointer;">' + svg(icon, 16, opts?.danger ? "#b02a37" : "#212529") + "</button>";
  }
  function activeCard(r, cat) {
    const m = CAT_META[cat];
    return '<div style="background:#fff;border:3px solid ' + m.accent + ';border-radius:16px;padding:14px 16px;margin-bottom:10px;box-shadow:0 4px 14px -6px rgba(16,24,40,.14);"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:' + m.accent + ';margin-bottom:2px;">Sedang Dipanggil</div><div style="display:flex;justify-content:space-between;align-items:center;gap:10px;"><b style="font-size:52px;line-height:1.05;letter-spacing:-.02em;color:' + m.accent + ';font-variant-numeric:tabular-nums;">' + r.queue_number + '</b><div style="text-align:right;min-width:0;"><div style="font-weight:700;font-size:17px;color:#212529;line-height:1.2;">' + (r.nama_pasien || "-") + '</div><div style="font-size:12px;color:#6c757d;">' + (r.counter?.name ? "Loket " + r.counter.name : "") + (r.called_at ? " \xB7 " + (r.called_at.slice(11, 16) || "") : "") + '</div></div></div><div style="display:flex;gap:6px;margin-top:10px;justify-content:flex-end;">' + iconBtn("RECALL", "recall", "Panggil ulang", r.queue_number, "op-recall-" + r.queue_number) + iconBtn("DEFER", "pause", "Tunda", r.queue_number, "op-defer-" + r.queue_number) + iconBtn("DONE", "check", "Selesai", r.queue_number, "op-done-" + r.queue_number) + "</div></div>";
  }
  function miniRow(r, prefix) {
    const actions = r.status === "WAITING" ? iconBtn("CALL", "speaker", "Panggil", r.queue_number, prefix + "-call-" + r.queue_number) + iconBtn(
      "PRINT",
      "printer",
      "Cetak tiket",
      r.queue_number,
      prefix + "-print-" + r.queue_number
    ) + iconBtn("DEFER", "pause", "Tunda", r.queue_number, prefix + "-defer-" + r.queue_number) + iconBtn("DONE", "check", "Selesai", r.queue_number, prefix + "-done-" + r.queue_number) : r.status === "CALLED" ? iconBtn(
      "RECALL",
      "recall",
      "Panggil ulang",
      r.queue_number,
      prefix + "-recall-" + r.queue_number
    ) + iconBtn("DEFER", "pause", "Tunda", r.queue_number, prefix + "-defer-" + r.queue_number) + iconBtn("DONE", "check", "Selesai", r.queue_number, prefix + "-done-" + r.queue_number) : iconBtn(
      "RECALL",
      "recall",
      "Panggil ulang",
      r.queue_number,
      prefix + "-recall-" + r.queue_number
    ) + (r.status === "DEFERRED" ? iconBtn(
      "BATAL",
      "trash",
    ...
    const badge = r.status === "WAITING" ? "" : '<span style="display:inline-flex;align-items:center;gap:5px;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;background:' + (STATUS_META[r.status]?.bg || "#e9ecef") + ";color:" + (STATUS_META[r.status]?.fg || "#495057") + ';margin-right:6px;"><span style="width:7px;height:7px;border-radius:50%;background:' + (STATUS_META[r.status]?.dot || "#495057") + ';display:inline-block;"></span>' + (STATUS_META[r.status]?.label || r.status) + "</span>";
    return '<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:#fff;border:1px solid #e9ecef;border-radius:10px;margin-bottom:6px;">' + badge + '<b style="font-size:15px;color:#212529;min-width:52px;">' + r.queue_number + '</b><span style="flex:1;font-size:13px;color:#495057;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + (r.nama_pasien || "-") + '</span><span style="display:flex;gap:4px;flex-shrink:0;">' + actions + "</span></div>";
  }
  function column(cat, active, next) {
    const m = CAT_META[cat];
    const nextList = next;
    const nextBtn = nextList.length ? '<button class="ext-op-act" data-ev="CALL" data-num="' + nextList[0].queue_number + '" data-eid="op-next-' + cat + '" data-tip="Panggil antrean berikutnya (' + nextList[0].queue_number + ')" title="Panggil antrean berikutnya" style="width:100%;margin-top:8px;padding:12px;border:none;border-radius:10px;background:' + m.accent + ';color:#fff;font-size:15px;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:8px;">' + svg("play", 16, "#fff") + "Selanjutnya \u2014 " + nextList[0].queue_number + "</button>" : "";
    return '<div style="background:#f1f3f5;border:1px solid #dee2e6;border-radius:16px;padding:12px;display:flex;flex-direction:column;min-width:0;min-height:0;height:100%;overflow:hidden;"><div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-shrink:0;"><span style="width:10px;height:10px;border-radius:50%;background:' + m.accent + ';"></span><b style="font-size:15px;color:#212529;">' + m.label + "</b></div>" + (active.length ? active.map((r) => activeCard(r, cat)).join("") : "") + (active.length ? "" : '<div style="padding:14px;background:#fff;border:1px dashed #ced4da;border-radius:12px;color:#6c757d;text-align:center;font-size:13px;margin-bottom:10px;flex-shrink:0;">Belum ada panggilan aktif</div>') + // "Berikutnya" section: label + scrollable list, button sticky di bawah.
    '<div style="display:flex;flex-direction:column;flex:1;min-height:0;max-height:350px;overflow:hidden;"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#6c757d;margin:4px 2px 6px;flex-shrink:0;">Berikutnya</div><div style="flex:1;min-height:0;overflow-y:auto;padding-right:4px;">' + (nextList.length ? nextList.map((r) => miniRow(r, "op-" + cat)).join("") : '<div style="padding:10px;color:#adb5bd;text-align:center;font-size:12px;">Tidak ada antrean berikutnya</div>') + // ponytail: button sticky di bawah scroll area — selalu terlihat meskipun banyak baris
    (nextList.length ? '<div style="position:sticky;bottom:0;padding:6px 0 2px;background:linear-gradient(transparent,#f1f3f5 30%);">' + nextBtn.replace("width:100%;margin-top:8px;", "width:100%;") + "</div>" : "") + "</div></div></div>";
  }
  function buildPanel() {
    const p = document.createElement("div");
    p.id = "ext-farmasi-operator";
    p.style.cssText = "padding:14px;max-width:1500px;margin:0 auto;font:14px/1.5 system-ui,sans-serif;color:#212529;background:#f8f9fa;height:100vh;box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;";
    p.innerHTML = '<style>#ext-farmasi-operator svg{display:inline-block !important;visibility:visible !important;width:16px;height:16px;flex:none;vertical-align:middle}#ext-farmasi-operator button{font-family:inherit}#ext-farmasi-operator button svg{pointer-events:none}#ext-farmasi-operator [data-tip]{position:relative}#ext-farmasi-operator [data-tip]:hover::after{content:attr(data-tip);position:absolute;bottom:calc(100% + 6px);left:50%;transform:translateX(-50%);background:#212529;color:#fff;font-size:11px;font-weight:600;line-height:1.4;white-space:nowrap;padding:4px 8px;border-radius:6px;z-index:99;box-shadow:0 2px 8px rgba(0,0,0,.25)}#ext-farmasi-operator [data-tip]:hover::before{content:"";position:absolute;bottom:calc(100% + 2px);left:50%;transform:translateX(-50%);border:4px solid transparent;border-top-color:#212529;z-index:99}</style><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px;flex-shrink:0;"><b style="font-size:18px;color:#2193cf;">Antrian Farmasi \u2014 Operasional</b><div id="ext-op-actions" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;"><span id="ext-op-status" style="color:#6c757d;font-size:12px;">memuat\u2026</span><button id="ext-op-print-sheet" data-tip="Cetak Sheet A4 \u2014 daftar hari ini atau kosong (T/R + jumlah)" style="padding:7px 14px;border:1px solid #2193cf;background:#2193cf;color:#fff;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' + svg("printer", 14, "#fff") + 'Cetak Sheet A4</button><button id="ext-op-set-counter" data-tip="Set nomor lanjutan setelah kendala (mati lampu dll) \u2014 antrian berikutnya lanjut dari nomor itu" style="padding:7px 14px;border:1px solid #0d6efd;background:#fff;color:#0d6efd;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' + svg("refresh", 14, "#0d6efd") + 'Set Nomor</button><button id="ext-op-delete-all" data-tip="Hapus SEMUA antrian hari ini dari DB (aksi permanen \u2014 tidak bisa dibatalkan)" style="padding:7px 14px;border:1px solid #b02a37;background:#fff;color:#b02a37;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' + svg("trash", 14, "#b02a37") + 'Hapus Semua</button><button id="ext-op-reset" data-tip="Reset antrian DB app \u2014 semua antrian hari ini kembali ke status awal, nomor dipanggil ulang dari T-01/R-01 (record tidak dihapus)" style="padding:7px 14px;border:1px solid #dc3545;background:#fff;color:#dc3545;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;font-weight:700;">' + svg("refresh", 14, "#dc3545") + 'Reset Antrian</button><button id="ext-op-display-antrian" data-tip="Tampilkan display PANGGILAN AKTIF di TV (frame /antrian-farmasi)" style="padding:7px 14px;border:1px solid #155e75;background:#155e75;color:#fff;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' + svg("volume", 14, "#fff") + 'Display Antrian</button><button id="ext-op-display-tunggu" data-tip="Tampilkan display ANTRIAN MENUNGGU di TV (frame /antrian-farmasi-menunggu \u2014 pasien lihat posisi antrean)" style="padding:7px 14px;border:1px solid #0d6efd;background:#0d6efd;color:#fff;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' + svg("list", 14, "#fff") + 'Display Tunggu</button><button id="ext-op-fullscreen" data-tip="Toggle fullscreen pada layar display TV (relay SSE)" style="padding:7px 14px;border:1px solid #6f42c1;background:#6f42c1;color:#fff;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' + svg("fullscreen", 14, "#fff") + 'Display FS</button><button id="ext-op-refresh" data-tip="Segarkan data antrean dari app" style="padding:7px 14px;border:1px solid #6c757d;background:#6c757d;color:#fff;border-radius:8px;cursor:pointer;">Segarkan</button></div></div><div id="ext-op-grid" style="display:grid;grid-template-columns:1fr 1fr 1.1fr;gap:12px;align-items:stretch;flex:1;min-height:0;overflow:hidden;"><div id="ext-col-tunggal"></div><div id="ext-col-racikan"></div><div id="ext-col-panel" style="background:#fff;border:1px solid #dee2e6;border-radius:16px;padding:12px;min-width:0;min-height:0;display:flex;flex-direction:column;overflow:hidden;"></div></div>';
    return p;
  }
  async function render() {
    ...
    try {
      ...
      try {
        const url = farmasiAppBase() + "/api/queue/display?limit=50" + (_opSig ? "&since=" + encodeURIComponent(_opSig) : "");
        ...
        if (key !== lastState) {
          ...
          if (colT && colR && colP) {
            ...
            const sortNum = (a, b) => a.queue_number.localeCompare(b.queue_number, void 0, { numeric: true });
            const byCat = (cat) => ({
              active: (d.current || []).filter((r) => catOf(r.queue_number) === cat).slice(0, 5),
              next: queues.filter((r) => catOf(r.queue_number) === cat && r.status === "WAITING").sort(sortNum)
            });
            ...
            const special = queues.filter((r) => r.status === "DEFERRED" || r.status === "SKIPPED").sort(sortNum);
            colP.innerHTML = '<div style="display:flex;flex-direction:column;min-height:0;height:100%;overflow:hidden;"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#6c757d;margin-bottom:8px;flex-shrink:0;">Penerbitan & Kasus Khusus</div><div style="display:flex;gap:8px;margin-bottom:12px;flex-shrink:0;"><button id="ext-op-print-sheet2" data-tip="Cetak daftar semua nomor antrian hari ini (format A4)" title="Cetak Sheet A4" style="flex:1;padding:9px;border:1px solid #2193cf;background:#2193cf;color:#fff;border-radius:8px;cursor:pointer;font-weight:700;display:inline-flex;align-items:center;justify-content:center;gap:6px;">' + svg("printer", 14, "#fff") + 'Sheet A4</button><button id="ext-op-refresh2" data-tip="Segarkan data antrean dari app" title="Segarkan" style="flex:1;padding:9px;border:1px solid #6c757d;background:#6c757d;color:#fff;border-radius:8px;cursor:pointer;font-weight:700;">Segarkan</button></div><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#6c757d;margin-bottom:6px;flex-shrink:0;">Ditunda / Lewat</div><div style="flex-shrink:0;max-height:170px;overflow-y:auto;padding-right:4px;">' + (special.length ? special.map((r) => miniRow(r, "op-sp")).join("") : '<div style="padding:10px;color:#adb5bd;text-align:center;font-size:12px;">Tidak ada</div>') + '</div><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#6c757d;margin:14px 0 6px;flex-shrink:0;">Selesai Hari Ini</div><div style="flex:1;min-height:0;overflow-y:auto;padding-right:4px;">' + (queues.filter((r) => r.status === "DONE").sort(sortNum).map((r) => miniRow(r, "op-done")).join("") || '<div style="padding:10px;color:#adb5bd;text-align:center;font-size:12px;">Belum ada</div>') + "</div></div>";
          }
        }
  ...
  async function act(ev, num, eid) {
    ...
    if (ev === "BATAL") {
      if (!confirm("Hapus antrian " + num + " dari DB? Record dihapus \u2014 resep bisa di-antrikan ulang.")) {
        return;
      }
    }
    const now = Date.now();
  ...
  function init() {
    const start = () => {
      ...
      document.getElementById("ext-op-delete-all")?.addEventListener("click", () => void deleteAllQueue());
      ...
      if (_fsPollTimer === null) {
        ...
        _fsPollTimer = window.setInterval(() => {
          fetch(FS_API + "/fullscreen-status?since=" + encodeURIComponent(fsSig), {
            cache: "no-store"
          }).then((r) => r.status === 304 ? null : r.json()).then((j) => {
            if (!j || !j.data) return;
      ...
      function fsFlash(msg) {
        ...
        if (!el) {
          ...
          el.style.cssText = "padding:4px 10px;border-radius:999px;font-size:12px;font-weight:700;background:#6f42c1;color:#fff;opacity:0;transition:opacity .25s ease;display:inline-flex;align-items:center;gap:6px;";
