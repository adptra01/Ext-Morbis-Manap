...
var __morbis_feature = (() => {
  ...
  function N(e, r, n, t) {
    ...
    a && a.remove();
    let s = r.toLowerCase().split('.').pop() || '',
      l = s === 'pdf',
      c = ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(s),
      i = document.createElement('div');
    ((i.id = 'ext-inline-preview-modal'),
      (i.style.cssText =
  ...
  `),
      ...
      document.getElementById('ext-preview-newtab')?.addEventListener('click', () => {
        (window.open(n || e, '_blank'), t && t(), i.remove());
      }),
      i.addEventListener('click', (p) => {
        p.target === i && (t && t(), i.remove());
      }),
      document.addEventListener('keydown', function p(m) {
        m.key === 'Escape' && (t && t(), i.remove(), document.removeEventListener('keydown', p));
      }),
      l || c)
    ) {
      let p = setInterval(() => {
        if (
          ...
          let y = i.querySelector('.ext-inline-preview-loading');
          (y && y.remove(), clearInterval(p));
        }
      }, 500);
    }
  }
...
`,
          ...
          .forEach((i, o) => {
            (o > 0 && a.appendChild(document.createElement('br')),
              a.appendChild(document.createTextNode(i)));
          });
      let s = (i) => {
          (t.remove(), document.removeEventListener('keydown', l), r(i));
        },
        l = (i) => {
          i.key === 'Escape' && s(!1);
        };
      ...
      (c && c.addEventListener('click', () => s(!1)),
  ...
  function q(e, r) {
    ...
    if (r) {
      let w = r
        ...
        S = (e.tanggal || '').replace(/\D/g, '').slice(0, 8),
        U = [C, S].filter(Boolean).join('_');
      return U ? `${U}_${w}${T}` : `${w}${T}`;
    }
    let t = n.match(/\.([A-Za-z0-9]+)$/),
      a = t ? n.slice(0, -t[0].length) : n,
      s = ['pdf', 'jpg', 'jpeg', 'png', 'gif'],
      l = (t?.[1] || '').toLowerCase(),
      c = s.find((w) => l.endsWith(w) && l.length > w.length),
      i = c ? '.' + c : t ? '.' + l : '',
      p =
        (c ? a : a.replace(/\.(pdf|jpe?g|png|gif)$/i, ''))
      ...
      y = (e.tanggal || '').replace(/\D/g, '').slice(0, 8),
      E = [m, y].filter(Boolean).join('_');
    return E ? `${E}_${p}${i}` : `${p}${i}`;
  }
  var I = z(),
    g = {
  ...
  function A() {
    ...
    return (console.warn('[Batch Upload] Input #tgl tidak ditemukan, pakai tanggal hari ini'), X());
  }
  function k(e) {
    return e
      .replace(/&/g, '&amp;')
  ...
  function ee(e, r = {}, n = 3e4) {
    let t = new AbortController(),
      a = setTimeout(() => t.abort(), n);
    return fetch(e, { ...r, signal: t.signal }).finally(() => clearTimeout(a));
  }
  async function O(e, r = {}, n = 2) {
    let t = null;
    for (let a = 0; a <= n; a++) {
      try {
        let s = await ee(e, r);
        if (s.ok || (s.status >= 400 && s.status < 500 && s.status !== 429)) return s;
        t = new Error(`HTTP ${s.status}: ${s.statusText}`);
      } catch (s) {
        ((t = s),
      ...
      a < n &&
        ...
        console.log(`[Batch Upload] Retry ${a + 1}/${n} for ${e}`));
    }
...
`,
  ...
  function _(e) {
    try {
      let r = new URL(e),
        t = decodeURIComponent(r.pathname).split('/').pop() || 'unknown',
        a = t.replace(/\.[^/.]+$/, ''),
        s = a.split(/[-_\s]+/),
        l = '',
        c = A(),
        i = s.findIndex((p) => /^\d{3,12}$/.test(p) && !/^\d{10}$/.test(p));
      i !== -1 && ((l = s[i]), s.splice(i, 1));
      let d =
        s
        ...
        (document
          ...
          document.querySelectorAll('input[name="ext-upload-mode"]').forEach((n) => {
            n.addEventListener('change', (t) => {
              ...
              (a.value === 'manual'
                ? (s && (s.style.display = 'block'), l && (l.style.display = 'none'))
                : (s && (s.style.display = 'none'), l && (l.style.display = 'block')),
                (u = []),
                L([]),
                x(''));
            });
  ...
  function H() {
    ...
    if (e) {
      ...
      let r = document.getElementById('ext-upload-search-input');
      r && (r.value = '');
  ...
  function L(e) {
    let r = document.getElementById(g.previewId),
    ...
    if (
      ...
      (i.innerHTML = `<strong class="preview-header-text">Preview (${c.length} dari ${e.length} dokumen, ${e.filter((o) => o.selected !== !1).length} dipilih):</strong>`),
      r && ((r.innerHTML = ''), r.appendChild(i)),
      c.length === 0)
    ) {
      ...
      <label class="ext-checkbox-label" style="flex:1;min-width:0;">
        ...
        <div style="flex: 1; min-width: 0;">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 8px;">
            <strong style="font-size: 13px; color: #000000; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${d + 1}. ${k(o.filename)}${w}</strong>
    ...
    `));
      let T = f.querySelector('.ext-checkbox'),
        ...
        S = f.querySelector('.ext-delete-single-btn'),
        U = (B) => {
          if (b) return;
          ((o.selected = B),
            T && (T.checked = B),
            B ? f.classList.add('selected') : f.classList.remove('selected'));
          let j = e.filter((Q) => Q.selected !== !1).length;
          ((i.innerHTML = `<strong class="preview-header-text">Preview (${j} Dokumen Dipilih):</strong>`),
            n && (n.disabled = j === 0));
        };
      (T?.addEventListener('change', (B) => U(B.target.checked)),
        S?.addEventListener('click', () => U(!1)));
  ...
  function W(e) {
    let r = document.getElementById(g.progressId);
    if (!r) return;
    let n = r.querySelector('.progress-fill');
    e > 0
      ? ((r.style.display = 'block'), n && (n.style.width = `${e}%`))
      : (r.style.display = 'none');
  }
  function x(e) {
    let r = document.getElementById(g.statusId);
    r && (r.textContent = e);
  }
  function $(e) {
    let r = [
  ...
  function ne() {
    let r = document.getElementById(g.textareaId)?.value.trim() || '';
    if (!r) {
      v({
        title: 'Tidak ada URL',
    ...
    ((u = n.map((t) => _(t))), L(u), x(`${n.length} URL siap diproses`));
  }
  async function oe() {
    let r = new URLSearchParams(window.location.search).get('id_visit');
    if (!r) {
      v({
    ...
    try {
      let t = `${window.location.origin}/admisi/pelaksanaan_pelayanan/dokumen-pasien?id_visit=${r}&page=85&id_kunjungan=`,
        a = await fetch(t);
      ...
      for (let o = 1; o < c.length; o++) {
        ...
        let y = m.startsWith('http') ? m : `${window.location.origin}${m}`,
          E = d.cells[1]?.textContent?.trim() || '',
          w = d.cells[2]?.textContent?.trim() || '',
          f = d.cells[3]?.textContent?.trim() || '',
          T = d.cells[4]?.textContent?.trim() || '';
        i.push({ url: y, filenameTabel: E, tglFile: f, tglUpload: T, keteranganTabel: w });
      }
      if (i.length === 0) {
  ...
  async function re(e, r) {
    (x(`Mengunduh: ${k(r)}...`), console.log('[Batch Upload] Fetching URL:', e));
    let n;
    try {
    ...
    let a = r.includes('.') ? '.' + r.split('.').pop() : '',
      s = r.replace(/[<>:"/\\|?*]/g, '_');
    return new File([t], s, { type: t.type || `application/${a.slice(1) || 'octet-stream'}` });
  }
  function ae() {
    let r = (document.getElementById('jenis')?.value || '').toUpperCase(),
      n = r.includes('INAP') ? 'RI' : r.includes('JALAN') ? 'RJ' : '';
    if (!n) return '';
  ...
  async function R(e, r) {
    try {
      let n = q(e, e.keterangan);
      x(`Download: ${k(e.filename)}...`);
      let t = await re(e.url, n),
        a = new FormData();
      (a.append('id_visit', r),
      ...
      let s = ae(),
        ...
        c = l.startsWith(s.trim()) ? l : `${s}${l}`;
      (a.append('keterangan', c.slice(0, 150)),
        x(`Upload: ${k(n)} (${(t.size / 1024).toFixed(0)} KB)...`));
      let i = await O(g.uploadEndpoint, { method: 'POST', body: a, credentials: 'same-origin' }, 2);
      if (!i.ok) {
        if (i.redirected)
        ...
        let d = await i.text().catch(() => ''),
          ...
          y = m ? `Server ${i.status}: ${m[1]}` : `Server ${i.status}: ${p || i.statusText}`;
        throw new Error(y);
      }
      let o = await i.text();
  ...
  async function V() {
    ...
    for (let o = 0; o < l; o++) {
      ...
      try {
        ...
        m.success
          ? ((d.status = 'success'), t++)
          : ((d.status = 'error'), (d.error = m.error), a++);
      } catch (m) {
        ((d.status = 'error'), (d.error = m.message), a++);
      }
      let p = ((o + 1) / l) * 100;
      (W(p), L(u));
    }
    ...
    let i = document.querySelector('#' + g.modalId + ' .ext-modal-buttons');
    if (i) {
      let o = `<button class="ext-btn ext-btn-purple" id="ext-reload-btn"><span style="display:inline-flex;align-items:center;gap:7px;">${h.refresh} Reload Halaman</span></button>`,
        d =
          a > 0
            ...
            : '';
      ((i.innerHTML = `<div style="display:flex;gap:8px;justify-content:flex-end;">${d}${o}</div>`),
        document
  ...
  function Z() {
    ...
    let e = u.filter((r) => r.selected !== !1).length;
    if (e === 0) {
      v({
        title: 'Tidak ada dokumen dipilih',
  ...
  async function se() {
    ...
    try {
      let n = `${window.location.origin}/admisi/pelaksanaan_pelayanan/dokumen-pasien?id_visit=${r}&page=85&id_kunjungan=`,
        t = await fetch(n);
      ...
      for (let i = 1; i < l.length; i++) {
        ...
        let m = p.startsWith('http') ? p : `${window.location.origin}${p}`,
          y = o.cells[1]?.textContent?.trim() || '',
          E = o.cells[2]?.textContent?.trim() || '',
          w = o.cells[3]?.textContent?.trim() || '',
          f = o.cells[4]?.textContent?.trim() || '';
        c.push({ url: m, filenameTabel: y, tglFile: w, tglUpload: f, keteranganTabel: E });
      }
      if (c.length === 0) {
        chrome.runtime
          .sendMessage({
      ...
      ((u = c.map((i) => {
        let o = _(i.url);
        return (
          (o.tglFileTabel = i.tglFile),
          (o.tglUploadTabel = i.tglUpload),
          (o.filename = i.filenameTabel || o.filename),
          (o.keterangan = i.keteranganTabel || o.filename || '-'),
          (o.selected = !1),
          o
        );
      })),
  ...
  async function le() {
    try {
      ...
      for (let l = 0; l < s; l++) {
        let c = a[l];
        ((c.status = 'uploading'), G(l, s, n, t, u));
        try {
          let i = await R(c, r);
          i.success
            ? ((c.status = 'success'), n++)
            : ((c.status = 'error'), (c.error = i.error), t++);
        } catch (i) {
          ((c.status = 'error'), (c.error = i.message), t++);
        }
        G(l + 1, s, n, t, u);
      }
    } catch (e) {
  ...
  function G(e, r, n, t, a) {
    chrome.runtime
      .sendMessage({
        ...
        data: {
          percent: (e / r) * 100,
          status: `Diproses: ${e}/${r} - Sukses: ${n}, Gagal: ${t}`,
          items: a,
          finished: e >= r,
        },
      })
  ...
  typeof I.featureModules < 'u' && I.featureModules !== null
