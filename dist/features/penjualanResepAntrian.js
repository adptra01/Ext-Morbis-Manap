...
var __morbis_feature = (() => {
  ...
  async function antrikanResep(idVisit, nomorResep, jenis, reader2) {
    ...
    const idPasien = reader2.get("id_pasien") || new URLSearchParams(location.search).get("norm") || "";
    ...
    const syncPromise = pushQueueEvent({
      event_id: queueEventId("enq", idVisit, idVisit + "-" + jenisLabel) + "-" + Date.now().toString(36),
      event: "ENQUEUE",
      resep_id: nomorResep,
      nama_pasien: nama,
  ...
  function resolveNamaPasien() {
    ...
    for (const el of labeled) {
      ...
      const next = el.nextElementSibling || el.parentElement?.querySelector("input, select") || el.parentElement?.nextElementSibling;
      const val = (next?.textContent || next?.value || "").trim();
      if (val) return val.toUpperCase();
    }
    const PAGE_KEYWORDS = /(resep|penjualan|antrian|farmasi|penerimaan|pendaftaran|detail|edit|input|rekap|daftar|shift|cetak|pembayaran|penyerahan|racik|racikan|obat|kasir|pilih|aturan|pakai|dosis|jumlah|satuan|harga|total|biaya|unit|depo|kekuatan|tipe|standar|kronis|klaim|inacbgs|batch|aksi|tambah|selesai|hapus|kembali|simpan)/i;
  ...
  function renderActionBar(state, code) {
    ...
    const klik = (jenis) => {
      ...
      antrikanResep(idVisit, nomorResep2, jenis, reader).then((c) => renderActionBar("issued", c)).catch((e) => alert("[MORBIS Ext] " + (e?.message || "gagal mengantrikan"))).finally(() => {
        if (btn) {
          btn.disabled = false;
  ...
  function addAntrianBar() {
    const findHost = () => {
      ...
      const td = Array.from(document.querySelectorAll('td[valign="top"]')).find(
      ...
      if (!document.getElementById("ext-antrian-fieldset")) {
        ...
        div.style.cssText = "position:fixed;top:60px;right:12px;z-index:9999;background:#fff;border:1px solid #ccc;border-radius:8px;padding:8px;box-shadow:0 2px 8px rgba(0,0,0,.15);";
        div.innerHTML = '<legend style="font-weight:700;margin-bottom:4px;display:block;">Antrian Farmasi</legend><div id="ext-antrian-bar" style="display:flex;flex-wrap:wrap;align-items:center;"></div>';
      ...
      const check = (attempt) => {
        ...
        void lookupAntrianAny(reader).then((info) => {
          if (isResepBatal(info?.status)) {
            bar.innerHTML = '<span style="color:#b02a37;font-weight:700;">Resep dibatalkan \u2014 antrian tidak tersedia</span>';
            return;
          }
  ...
  function blockAutoAntrol() {
    const isAntrolCall = (url, body) => String(url ?? "").includes("/v2/antrol/search") && String(url ?? "").includes("sub=update_v2") && String(body ?? "").includes("taskid=6");
