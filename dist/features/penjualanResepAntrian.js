...
var __morbis_feature = (() => {
  function k(e) {
    return new Promise((r, n) => {
      chrome.runtime.sendMessage(e, (o) => {
        chrome.runtime.lastError ? n(chrome.runtime.lastError) : r(o);
      });
    });
  }
  var B = 'http://dev.rsudkotajambi.id/rs',
    H = null,
    g = null;
  async function F() {
    try {
  ...
  var D = ['http://dev.rsudkotajambi.id/rs', 'http://103.147.236.138/rs'],
    j = ['dev.rsudkotajambi.id', '103.147.236.138', 'localhost', '127.0.0.1'],
    z = ['.rsudkotajambi.id', '.ddev.site'];
  function X(e) {
    try {
      let r = new URL(e);
      if (r.protocol !== 'http:' && r.protocol !== 'https:') return !1;
      let n = r.hostname.toLowerCase();
      return j.includes(n) ? !0 : z.some((o) => n.endsWith(o));
    } catch {
      return !1;
    }
  }
  async function $(e, r, n) {
    return k({ type: 'QUEUE_API', url: e, method: r, body: n });
  }
  function Q(e, r) {
    return new Promise((n, o) => {
      let i = setTimeout(() => o(new Error('timeout')), r);
      e.then((t) => {
        (clearTimeout(i), n(t));
      }).catch((t) => {
        (clearTimeout(i), o(t));
    ...
    return (
      ...
      ((g = (async () => {
        try {
          ...
          if (n && X(n)) return n.replace(/\/+$/, '');
        } catch {}
        let e = await F(),
          r = [...new Set([...e, ...D])];
        for (let n of r)
          try {
            let o = await Q($(n + '/api/queue/lookup?resep_id=probe', 'GET'), 2500),
              i = o.contentType || '';
            if ((o.status === 200 || o.status === 422) && i.includes('application/json'))
              return ((H = n), n);
          } catch {}
        return B;
      })()),
  ...
  async function b(e) {
    try {
      let r = { ...e };
      if ((e.event === 'ENQUEUE' && delete r.queue_number, e.event === 'BATAL' && !e.queue_number))
        return (console.warn('[MORBIS Ext] BATAL tanpa queue_number \u2014 dilewati'), { ok: !1 });
      let n = await f(),
        o = new AbortController(),
        i = setTimeout(() => o.abort(), 8e3),
        t = await fetch(n + '/api/queue/events', {
          method: 'POST',
      ...
      if ((clearTimeout(i), !t.ok)) {
        let s = '';
        try {
          s = (await t.json())?.message || '';
        } catch {}
        throw new Error('HTTP ' + t.status + (s ? ' \u2014 ' + s : ''));
      }
      let a = await t.json();
      return {
        ok: !!a.ok,
  ...
  async function Y(e) {
    try {
      let r = (await chrome.storage.local.get(c))[c] ?? [];
      if (r.some((n) => n.event_id === e.event_id)) return;
      (r.push(e),
        r.length > K && r.shift(),
        await chrome.storage.local.set({ [c]: r }),
    ...
    if (e.length)
      for (let r of [...e])
        try {
          (await V(r)).ok &&
            (await A(r.event_id),
            console.log('[MORBIS Ext] retry queue sukses:', r.event, r.queue_number ?? ''));
        } catch (n) {
          let o = n.message ?? '';
  ...
  async function V(e) {
    let r = { ...e };
    e.event === 'ENQUEUE' && delete r.queue_number;
    let n = await f(),
      o = new AbortController(),
      i = setTimeout(() => o.abort(), 8e3),
      t = await fetch(n + '/api/queue/events', {
    ...
    if ((clearTimeout(i), !t.ok)) throw new Error('HTTP ' + t.status);
    let a = await t.json();
  ...
  function h(e, r, n) {
    return `${e}-${r}-${n}-${new Date().toISOString().slice(0, 10)}`;
  }
  function L(e, r = 5e3) {
    let n = document.documentElement,
      ...
      i = window.setInterval(() => {
        n.getAttribute('data-ext-antrian-farmasi') === '1'
          ...
          : Date.now() - o > r && (window.clearInterval(i), Z());
      }, 200);
  }
  function Z() {
...
`,
      ...
      let t = (l) => {
          (o.remove(), document.removeEventListener('keydown', a), r(l));
        },
        a = (l) => {
          l.key === 'Escape' && t(!1);
        };
      (o.querySelector('.ext-modal-close').addEventListener('click', () => t(!1)),
        o.addEventListener('click', (l) => {
          l.target === o && t(!1);
        }),
        o.querySelector('[data-ext-ok]').addEventListener('click', () => t(!0)));
      let s = o.querySelector('[data-ext-cancel]');
      (s && s.addEventListener('click', () => t(!1)),
  ...
  function y(e) {
    let r = window.open('', '_blank', 'width=400,height=560');
    if (!r)
      return (
        C({
  ...
  var te = '/v2/antrol/search',
    ne = 'sub=update_v2',
    re = '/public/antrian-farmasi-v2/list-antrian-v2';
  async function oe(e) {
    try {
      let r = await fetch((await f()) + '/api/queue/lookup?resep_id=' + encodeURIComponent(e), {
        cache: 'no-store',
        credentials: 'omit',
      });
      if (!r.ok) return null;
  ...
  function I(e) {
    ...
    try {
      ...
      for (let i of o) {
        let t = (i.textContent || '').trim();
        if (
          /^(batal|dibatalkan|resep batal|sudah dibatalkan)$/i.test(t) &&
  ...
  async function ie(e) {
    return fetch(`${te}?${ne}`, {
      ...
      .then((r) => (console.log('[MORBIS Ext] antrian terdaftar id=' + e, 'status', r.status), !0))
      .catch((r) => (console.warn('[MORBIS Ext] gagal mendaftarkan antrian', r), !1));
  }
  async function ae(e, r) {
    try {
      let n = await fetch(re, {
        method: 'POST',
      ...
      let i = String(r ?? '').slice(0, 16);
      return (
        o.find(
          (t) =>
            String(t.ID_PASIEN ?? '') === String(e) &&
            (!i || String(t.WAKTU ?? '').slice(0, 16) === i),
        ) ??
        o.find((t) => String(t.ID_PASIEN ?? '') === String(e)) ??
        null
      );
    } catch {
      return null;
  ...
  async function O(e, r, n, o) {
    if (!(await ie(e))) throw new Error('gagal antrol MORBIS');
    let t = o.get('id_pasien') || new URLSearchParams(location.search).get('norm') || '',
      a = o.get('waktu_pengajuan'),
      s = o.namaPasien(),
      l = n === 'racik' ? 'racikan' : 'tunggal',
      d = (async () => {
        for (let v = 0; v < 3; v++) {
          let T = await ae(t, a);
          if (T) return T;
      ...
      q = b({
        event_id: h('enq', e, e + '-' + l) + '-' + Date.now().toString(36),
        event: 'ENQUEUE',
        resep_id: r,
        nama_pasien: s,
        norm: t || void 0,
        tgl_lahir: o.tglLahir() || void 0,
        shift: '',
        jenis: l,
        counter: '',
        payload: { idVisit: e, unit: '', waktu: a || '' },
      }),
      [G, w] = await Promise.all([d, q]);
  ...
  async function P(e, r, n) {
    let o = h('bat', r, e) + '-' + Date.now().toString(36);
    return (await b({ event_id: o, event: 'BATAL', queue_number: e, resep_id: r })).ok
      ? { ok: !0, gone: !1 }
      : (await E(n))
        ? { ok: !1, gone: !1 }
        : { ok: !0, gone: !0 };
  }
  function U(e, r, n, o = '', i = '', t = '') {
    y({
      nomorResep: e,
      nama: n.namaPasien(),
      jenis: o,
  ...
  if (window.__extPenjualanAntrian) throw new Error('skip double inject penjualanResepAntrian');
  window.__extPenjualanAntrian = !0;
  ...
  function ce() {
    ...
    for (let t of i) {
      if (
        t.closest('.modal, .modal-header, .modal-body, .dropdown, .dropdown-menu, [role="dialog"]')
      )
        continue;
      let a = (t.textContent || '').trim();
      if (!(
  ...
  function de() {
    ...
    for (let n of r) {
      ...
      for (let i = 0; i < o.length - 1; i++)
        if (/^tanggal\s*lahir$/i.test(o[i].textContent?.trim() || '') && o[i + 1]) {
          let t = o[i + 1].textContent?.trim();
          if (t && t !== ':') return t;
        }
    }
    return '';
  ...
  function u(e, r) {
    if (e) {
      let n = document.querySelector('#' + e)?.value?.trim() || '';
      if (n) return n;
    }
    if (r) {
      let n = document.querySelector('input[name="' + r + '"]')?.value?.trim() || '';
      if (n) return n;
    }
    return e === 'id_resep' ? le : e === 'id_visit' ? se : '';
  }
  var m = { get: u, namaPasien: ce, tglLahir: de };
  function p(e, r) {
    let n = document.querySelector('#ext-antrian-bar');
    if (!n) return;
    let o = u('nomor_resep', 'id_resep');
    if (e === 'issued' && r) {
      ((n.innerHTML =
        ...
        n.querySelector('#ext-antrian-batal')?.addEventListener('click', async () => {
          if (!confirm('Batalkan antrian ' + r + '? Resep akan keluar dari daftar panggilan.'))
            return;
          let t = n.querySelector('#ext-antrian-batal');
          if (
            (t && ((t.disabled = !0), (t.textContent = 'Membatalkan\u2026')),
            !(await P(r || '', o, m)).ok)
          ) {
            (alert('[MORBIS Ext] Gagal membatalkan antrian. Coba lagi.'),
              t && ((t.disabled = !1), (t.textContent = 'Batal antrian')));
            return;
          }
          p('ready');
        }));
    ...
    let i = (t) => {
      ...
      let l = document.querySelector(t === 'racik' ? '#ext-antrian-racik' : '#ext-antrian-tunggal');
      (l && ((l.disabled = !0), (l.textContent = 'Memproses\u2026')),
        O(a, s, t, m)
          .then((d) => p('issued', d))
          ...
          .finally(() => {
            l &&
              ((l.disabled = !1),
              (l.textContent = t === 'racik' ? 'Antrikan obat racik' : 'Antrikan obat tunggal'));
          }));
    };
  ...
  function ue() {
    let e = () => {
        ...
        let n = Array.from(document.querySelectorAll('td[valign="top"]')).find((t) =>
          t.querySelector('fieldset#perhatian, fieldset[id="perhatian"]'),
        );
        if (n) {
          let t = document.createElement('fieldset');
          ((t.id = 'ext-antrian-fieldset'),
            (t.style.cssText = 'margin-top:6px;'),
            (t.innerHTML = '<legend>Antrian Farmasi</legend>'));
          let a = document.createElement('div');
          return (
            (a.id = 'ext-antrian-bar'),
            (a.style.cssText = 'display:flex;flex-wrap:wrap;align-items:center;'),
            t.appendChild(a),
            n.appendChild(t),
            a
          );
        }
        let o = document.querySelector('form');
        if (o) {
          let t = document.createElement('fieldset');
          ((t.id = 'ext-antrian-fieldset'),
            (t.style.cssText = 'margin-top:6px;'),
          ...
          ((a.id = 'ext-antrian-bar'),
            (a.style.cssText = 'display:flex;flex-wrap:wrap;align-items:center;'),
            t.appendChild(a));
          let s = o.querySelector('button[type="submit"], input[type="submit"]');
          return (s ? s.parentElement?.insertBefore(t, s) : o.prepend(t), a);
        }
        let i = document.querySelector('.card-body, .panel-body, .form-horizontal');
        if (i) {
          let t = document.createElement('fieldset');
          ((t.id = 'ext-antrian-fieldset'),
            (t.style.cssText = 'margin-top:6px;'),
        ...
        if (!document.getElementById('ext-antrian-fieldset')) {
          ...
          return (
            ...
            t.querySelector('#ext-antrian-bar')
          );
        }
        return null;
      },
      r = () => {
        let o = document.querySelector('#ext-antrian-bar') || e();
        if (!o) return;
        let i = (t) => {
          if (!u('id_resep', 'nomor_resep')) {
            t < 10 ? window.setTimeout(() => i(t + 1), 800) : p('ready');
            return;
          }
          E(m).then((s) => {
            if (I(s?.status)) {
            ...
            s
              ? p('issued', s.queue_number)
              : t < 10
                ? window.setTimeout(() => i(t + 1), 800)
                : p('ready');
          });
        };
        i(0);
  ...
  function pe() {
    let e = (i, t) =>
        ...
        String(t ?? '').includes('taskid=6'),
      r = XMLHttpRequest.prototype.open,
      n = XMLHttpRequest.prototype.send;
    ((XMLHttpRequest.prototype.open = function (i, t, ...a) {
      return ((this.__extUrl = String(t)), r.apply(this, [i, t, ...a]));
    }),
    ...
    let o = window.fetch.bind(window);
    window.fetch = (i, t) => {
