...
var __morbis_feature = (() => {
  ...
  async function crawlDokumenPasienDelete() {
    ...
    try {
      ...
      const response = await fetch(targetUrl, { signal: AbortSignal.timeout(3e4) });
...
Tindakan ini tidak bisa di-undo.`,
  ...
  function updateDeletePreview() {
    ...
    header.style.cssText = "padding:10px 16px;background:#f8fafc;border-bottom:1px solid #f1f5f9;font-size:11px;font-weight:700;color:#1e293b;text-transform:uppercase;letter-spacing:0.5px;";
    const updateHeader = () => {
      header.innerHTML = 'Dokumen Pasien <span style="color:#64748b;font-weight:400;">(' + deleteQueue.length + ' dokumen, <span style="color:#dc2626;">' + deleteQueue.filter((i) => i.selected).length + "</span> dipilih)</span>";
    };
    updateHeader();
    previewEl.innerHTML = "";
