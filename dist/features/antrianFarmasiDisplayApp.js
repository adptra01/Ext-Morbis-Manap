...
var __morbis_feature = (() => {
  ...
  async function I(n) {
    ...
    let T = await i.json();
    return { ok: !!T.ok, queue_number: T.queue?.queue_number };
  }
  setInterval(() => {
    w();
  }, 1e4);
  ...
  var p = {
    calls: { url: _() + '/antrian-farmasi', label: 'panggilan aktif' },
    waiting: { url: _() + '/antrian-farmasi-menunggu', label: 'antrian menunggu' },
  };
  (function () {
    let n = 'calls',
      e = null,
    ...
    function s() {
      try {
        ...
        let r = () => {
          document.querySelectorAll('audio, video').forEach((d) => {
            let u = d;
            ((u.muted = !0), u.pause());
          });
        };
    ...
    function i() {
      ...
      try {
        let d = new BroadcastChannel('morbis-antrian-display');
        d.onmessage = (u) => {
