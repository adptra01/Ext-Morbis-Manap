(function () {
  var ENHANCER_ACTIVE = false;
  var ENHANCER_OBSERVER = null;
  var MAX_WAIT = 100;
  var waited = 0;

  var check = setInterval(function () {
    waited++;
    var enabled = document.documentElement.getAttribute('data-ext-consul-enhancer');
  ...
  function run() {
    ...
    jqScript.onload = function () {
      ...
      var oldJq = window.jQuery.noConflict(true);
      fetch('https://cdn.datatables.net/1.13.6/js/jquery.dataTables.min.js')
        .then(function (r) { return r.text(); })
        .then(function (code) {
          window.jQuery = jq3;
          var dtInline = document.createElement('script');
        ...
        .catch(function () {
          ...
          dtScript.onload = function () {
            var jq3b = window.jQuery.noConflict(true);
            init(jq3b);
          };
  ...
  function init($) {
    ...
    document.querySelectorAll('table.tabel.full').forEach(function (tbl) {
      ...
      var hcells = theadRow ? theadRow.querySelectorAll('th, td') : [];
      var headerTexts = [];
      hcells.forEach(function (c) { if (c.textContent.trim() !== '') headerTexts.push(c.textContent.trim()); });
      ...
      var thCount = theadRow ? theadRow.querySelectorAll('th, td').length : 0;
      var tdCount = tbl.querySelector('tbody tr:first-child') ? tbl.querySelector('tbody tr:first-child').cells.length : 0;
      ...
      tbl.querySelectorAll('tbody tr').forEach(function (tr) {
        var btn = tr.querySelector('button[onclick*="direction_konsul"], button[onclick*="edit"]');
    ...
    ENHANCER_OBSERVER = new MutationObserver(function () {
      var fresh = document.querySelectorAll('table.tabel.full:not(.dataTable)');
      if (fresh.length > 0) {
  ...
  function addButtons(api, permintaanIdx, kesanIdx, anjuranIdx) {
    var totalCols = api.columns().count();
    var actionCol = totalCols - 1;
    ...
    api.cells(null, actionCol).every(function () {
      ...
      i.onclick = function () {
        var oc = row.getAttribute('data-onclick') || '';
        var m = oc.match(/'(.*?)'/g);
        if (!m || m.length < 2) return;
        var id = m[0].replace(/'/g, '');
        var visit = m[1].replace(/'/g, '');
        var poli = m[2] ? m[2].replace(/'/g, '') : '';
        var rd = api.row(row).data();
        openInfo({
          id: id, visit: visit, poli: poli,
          nama: rd ? rd[2] || '' : '',
          noRm: rd ? rd[1] || '' : ''
        });
      };
  ...
  function openDetail(data) {
    var el = document.getElementById('morbis-cons-modal');
    if (el) el.remove();

    var f =
      '<label>No. RM</label><div class="morbis-cons-fv">' + esc(data.noRm) + ' \u2014 ' + esc(data.nama) + '</div>' +
      '<label>Unit Asal \u2192 Unit Tujuan</label><div class="morbis-cons-fv">' + esc(data.unitAsal) + ' \u2192 ' + esc(data.unitTujuan) + '</div>' +
      '<label>Dokter</label><div class="morbis-cons-fv">' + esc(data.dokterMengajukan) + ' \u2192 ' + esc(data.dokterKonsultasi) + '</div>' +
      '<label>Tanggal Pengajuan</label><div class="morbis-cons-fv">' + esc(data.tanggal) + '</div>';

    if (data.permintaan !== undefined) {
      f += '<label>Permintaan Konsultasi</label><div class="morbis-cons-fv">' + esc(data.permintaan) + '</div>';
    }
    ...
    ov.innerHTML =
      '<div class="morbis-cons-content">' +
        '<div class="morbis-cons-header">' +
          '<h2>Detail Konsultasi \u2014 ' + esc(data.nama) + '</h2>' +
          '<button class="morbis-cons-close">&times;</button>' +
        '</div>' +
        '<div class="morbis-cons-body">' + f + '</div>' +
      '</div>';

    document.body.appendChild(ov);
    ov.querySelector('.morbis-cons-close').onclick = function () { ov.remove(); };
    ov.onclick = function (e) { if (e.target === ov) ov.remove(); };
  }

  function openInfo(data) {
    var url = window.location.origin + '/admisi/pengajuan_konsultasi/form-input-konsultasi/?id=' + data.id + '&id_visit=' + data.visit + '&poli=' + data.poli;

    var el = document.getElementById('morbis-cons-info');
    if (el) el.remove();
    ...
    ov.innerHTML =
      '<div class="morbis-cons-content" style="width:95%;max-width:1200px;">' +
        '<div class="morbis-cons-header">' +
          '<h2>Informasi Pasien \u2014 ' + esc(data.nama) + ' (' + esc(data.noRm) + ')</h2>' +
          '<button class="morbis-cons-close">&times;</button>' +
        '</div>' +
