"use strict";var __morbis_feature=(()=>{function k(e){return new Promise((r,n)=>{chrome.runtime.sendMessage(e,o=>{chrome.runtime.lastError?n(chrome.runtime.lastError):r(o)})})}var B="http://dev.rsudkotajambi.id/rs",H=null,g=null;async function F(){try{return((await chrome.storage.sync.get("extensionCustomUrls")).extensionCustomUrls??[]).filter(n=>n.url&&n.enabled!==!1).map(n=>n.url.replace(/\/+$/,"")+"/rs")}catch{return[]}}var D=["http://dev.rsudkotajambi.id/rs","http://103.147.236.138/rs"],j=["dev.rsudkotajambi.id","103.147.236.138","localhost","127.0.0.1"],z=[".rsudkotajambi.id",".ddev.site"];function X(e){try{let r=new URL(e);if(r.protocol!=="http:"&&r.protocol!=="https:")return!1;let n=r.hostname.toLowerCase();return j.includes(n)?!0:z.some(o=>n.endsWith(o))}catch{return!1}}async function $(e,r,n){return k({type:"QUEUE_API",url:e,method:r,body:n})}function Q(e,r){return new Promise((n,o)=>{let i=setTimeout(()=>o(new Error("timeout")),r);e.then(t=>{clearTimeout(i),n(t)}).catch(t=>{clearTimeout(i),o(t)})})}function f(){return g||(g=(async()=>{try{let n=localStorage.getItem("ext-farmasi-app-base");if(n&&X(n))return n.replace(/\/+$/,"")}catch{}let e=await F(),r=[...new Set([...e,...D])];for(let n of r)try{let o=await Q($(n+"/api/queue/lookup?resep_id=probe","GET"),2500),i=o.contentType||"";if((o.status===200||o.status===422)&&i.includes("application/json"))return H=n,n}catch{}return B})(),g)}var _="";async function b(e){try{let r={...e};if(e.event==="ENQUEUE"&&delete r.queue_number,e.event==="BATAL"&&!e.queue_number)return console.warn("[MORBIS Ext] BATAL tanpa queue_number \u2014 dilewati"),{ok:!1};let n=await f(),o=new AbortController,i=setTimeout(()=>o.abort(),8e3),t=await fetch(n+"/api/queue/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r),cache:"no-store",credentials:"omit",signal:o.signal});if(clearTimeout(i),!t.ok){let s="";try{s=(await t.json())?.message||""}catch{}throw new Error("HTTP "+t.status+(s?" \u2014 "+s:""))}let a=await t.json();return{ok:!!a.ok,queue_number:a.queue?.queue_number,created:a.created,duplicate:a.duplicate}}catch(r){let n=r.message;return n!==_&&(console.warn("[MORBIS Ext] queue sync gagal:",n),_=n),await Y(e),{ok:!1}}}var c="ext-queue-retry-queue",K=20;async function Y(e){try{let r=(await chrome.storage.local.get(c))[c]??[];if(r.some(n=>n.event_id===e.event_id))return;r.push(e),r.length>K&&r.shift(),await chrome.storage.local.set({[c]:r}),console.log("[MORBIS Ext] disimpan ke retry queue:",e.event,e.queue_number??"")}catch{}}async function W(){try{return(await chrome.storage.local.get(c))[c]??[]}catch{return[]}}async function A(e){try{let n=((await chrome.storage.local.get(c))[c]??[]).filter(o=>o.event_id!==e);await chrome.storage.local.set({[c]:n})}catch{}}async function J(){let e=await W();if(e.length)for(let r of[...e])try{(await V(r)).ok&&(await A(r.event_id),console.log("[MORBIS Ext] retry queue sukses:",r.event,r.queue_number??""))}catch(n){let o=n.message??"";(o.includes("HTTP 404")||o.includes("HTTP 422"))&&(await A(r.event_id),console.log("[MORBIS Ext] retry queue buang (stale):",r.event,r.queue_number??"",o))}}async function V(e){let r={...e};e.event==="ENQUEUE"&&delete r.queue_number;let n=await f(),o=new AbortController,i=setTimeout(()=>o.abort(),8e3),t=await fetch(n+"/api/queue/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r),cache:"no-store",credentials:"omit",signal:o.signal});if(clearTimeout(i),!t.ok)throw new Error("HTTP "+t.status);let a=await t.json();return{ok:!!a.ok,queue_number:a.queue?.queue_number}}setInterval(()=>{J()},1e4);function h(e,r,n){return`${e}-${r}-${n}-${new Date().toISOString().slice(0,10)}`}function L(e,r=5e3){let n=document.documentElement,o=Date.now(),i=window.setInterval(()=>{n.getAttribute("data-ext-antrian-farmasi")==="1"?(window.clearInterval(i),e()):Date.now()-o>r&&(window.clearInterval(i),Z())},200)}function Z(){if(!document.body||document.getElementById("ext-feature-gate-notif"))return;let e=document.createElement("div");e.id="ext-feature-gate-notif",e.textContent="\u26A0\uFE0F Fitur antrian tidak aktif \u2014 muat ulang halaman (F5)",e.style.cssText="position:fixed;top:8px;right:8px;z-index:999999;background:#dc3545;color:#fff;padding:8px 16px;border-radius:6px;font:13px system-ui,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.2);",document.body.appendChild(e),setTimeout(()=>e.remove(),1e4)}var C="ext-batch-shared-style";function ee(){if(document.getElementById(C))return;let e=document.createElement("style");e.id=C,e.textContent=`
    .ext-modal-content {
