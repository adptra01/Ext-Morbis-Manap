"use strict";var __morbis_feature=(()=>{function T(e){return new Promise((t,n)=>{chrome.runtime.sendMessage(e,r=>{chrome.runtime.lastError?n(chrome.runtime.lastError):t(r)})})}var M="http://dev.rsudkotajambi.id/rs",B=null,p=null;async function H(){try{return((await chrome.storage.sync.get("extensionCustomUrls")).extensionCustomUrls??[]).filter(n=>n.url&&n.enabled!==!1).map(n=>n.url.replace(/\/+$/,"")+"/rs")}catch{return[]}}var F=["http://dev.rsudkotajambi.id/rs","http://103.147.236.138/rs"],D=["dev.rsudkotajambi.id","103.147.236.138","localhost","127.0.0.1"],j=[".rsudkotajambi.id",".ddev.site"];function z(e){try{let t=new URL(e);if(t.protocol!=="http:"&&t.protocol!=="https:")return!1;let n=t.hostname.toLowerCase();return D.includes(n)?!0:j.some(r=>n.endsWith(r))}catch{return!1}}async function X(e,t,n){return T({type:"QUEUE_API",url:e,method:t,body:n})}function $(e,t){return new Promise((n,r)=>{let i=setTimeout(()=>r(new Error("timeout")),t);e.then(o=>{clearTimeout(i),n(o)}).catch(o=>{clearTimeout(i),r(o)})})}function g(){return p||(p=(async()=>{try{let n=localStorage.getItem("ext-farmasi-app-base");if(n&&z(n))return n.replace(/\/+$/,"")}catch{}let e=await H(),t=[...new Set([...e,...F])];for(let n of t)try{let r=await $(X(n+"/api/queue/lookup?resep_id=probe","GET"),2500),i=r.contentType||"";if((r.status===200||r.status===422)&&i.includes("application/json"))return B=n,n}catch{}return M})(),p)}var _="";async function b(e){try{let t={...e};if(e.event==="ENQUEUE"&&delete t.queue_number,e.event==="BATAL"&&!e.queue_number)return console.warn("[MORBIS Ext] BATAL tanpa queue_number \u2014 dilewati"),{ok:!1};let n=await g(),r=new AbortController,i=setTimeout(()=>r.abort(),8e3),o=await fetch(n+"/api/queue/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t),cache:"no-store",credentials:"omit",signal:r.signal});if(clearTimeout(i),!o.ok){let s="";try{s=(await o.json())?.message||""}catch{}throw new Error("HTTP "+o.status+(s?" \u2014 "+s:""))}let a=await o.json();return{ok:!!a.ok,queue_number:a.queue?.queue_number,created:a.created,duplicate:a.duplicate}}catch(t){let n=t.message;return n!==_&&(console.warn("[MORBIS Ext] queue sync gagal:",n),_=n),await K(e),{ok:!1}}}var c="ext-queue-retry-queue",Q=20;async function K(e){try{let t=(await chrome.storage.local.get(c))[c]??[];if(t.some(n=>n.event_id===e.event_id))return;t.push(e),t.length>Q&&t.shift(),await chrome.storage.local.set({[c]:t}),console.log("[MORBIS Ext] disimpan ke retry queue:",e.event,e.queue_number??"")}catch{}}async function Y(){try{return(await chrome.storage.local.get(c))[c]??[]}catch{return[]}}async function A(e){try{let n=((await chrome.storage.local.get(c))[c]??[]).filter(r=>r.event_id!==e);await chrome.storage.local.set({[c]:n})}catch{}}async function W(){let e=await Y();if(e.length)for(let t of[...e])try{(await J(t)).ok&&(await A(t.event_id),console.log("[MORBIS Ext] retry queue sukses:",t.event,t.queue_number??""))}catch(n){let r=n.message??"";(r.includes("HTTP 404")||r.includes("HTTP 422"))&&(await A(t.event_id),console.log("[MORBIS Ext] retry queue buang (stale):",t.event,t.queue_number??"",r))}}async function J(e){let t={...e};e.event==="ENQUEUE"&&delete t.queue_number;let n=await g(),r=new AbortController,i=setTimeout(()=>r.abort(),8e3),o=await fetch(n+"/api/queue/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t),cache:"no-store",credentials:"omit",signal:r.signal});if(clearTimeout(i),!o.ok)throw new Error("HTTP "+o.status);let a=await o.json();return{ok:!!a.ok,queue_number:a.queue?.queue_number}}setInterval(()=>{W()},1e4);function h(e,t,n){return`${e}-${t}-${n}-${new Date().toISOString().slice(0,10)}`}function L(e,t=5e3){let n=document.documentElement,r=Date.now(),i=window.setInterval(()=>{n.getAttribute("data-ext-antrian-farmasi")==="1"?(window.clearInterval(i),e()):Date.now()-r>t&&(window.clearInterval(i),V())},200)}function V(){if(!document.body||document.getElementById("ext-feature-gate-notif"))return;let e=document.createElement("div");e.id="ext-feature-gate-notif",e.textContent="\u26A0\uFE0F Fitur antrian tidak aktif \u2014 muat ulang halaman (F5)",e.style.cssText="position:fixed;top:8px;right:8px;z-index:999999;background:#dc3545;color:#fff;padding:8px 16px;border-radius:6px;font:13px system-ui,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.2);",document.body.appendChild(e),setTimeout(()=>e.remove(),1e4)}var C="ext-batch-shared-style";function Z(){if(document.getElementById(C))return;let e=document.createElement("style");e.id=C,e.textContent=`
    .ext-modal-content {
...
`).forEach((l,d)=>{d>0&&i.appendChild(document.createElement("br")),i.appendChild(document.createTextNode(l))});let o=l=>{r.remove(),document.removeEventListener("keydown",a),t(l)},a=l=>{l.key==="Escape"&&o(!1)};r.querySelector(".ext-modal-close").addEventListener("click",()=>o(!1)),r.addEventListener("click",l=>{l.target===r&&o(!1)}),r.querySelector("[data-ext-ok]").addEventListener("click",()=>o(!0));let s=r.querySelector("[data-ext-cancel]");s&&s.addEventListener("click",()=>o(!1)),document.addEventListener("keydown",a),document.body.appendChild(r)})}var S="RSUD H. Abdul Manap";function y(e){let t=window.open("","_blank","width=400,height=560");if(!t)return R({title:"Popup Diblokir",message:"Izinkan popup untuk mencetak.",variant:"warning",okLabel:"OK",hideCancel:!0}),!1;let n=e.jenis||e.unit?`<div style="font-size:16px;margin-top:2px;">${[e.jenis,e.unit].filter(Boolean).join(" \xB7 ")}</div>`:"",r=e.tglLahir?`<div style="font-size:13px;margin-top:4px;color:#555;">${e.tglLahir}</div>`:"";return t.document.write('<html><head><title>Antrian Farmasi</title></head><body style="width:320px;padding-top:10px;font-family:Arial,Helvetica,sans-serif;text-align:center;"><div style="font-size:16px;font-weight:bold;text-transform:uppercase;">'+S+`</div><div style="font-size:14px;margin-top:2px;">Antrian Farmasi</div><div style="margin-top:14px;"><div style="font-size:110px;font-weight:900;letter-spacing:-2px;line-height:1;">${e.code}</div></div><div style="font-size:20px;font-weight:bold;margin-top:10px;">${e.nama}</div>`+r+n+`<div style="font-size:11px;margin-top:10px;color:#333;">${e.tanggal}</div><div style="font-size:13px;margin-top:14px;color:#555;">Silakan menunggu panggilan</div></body></html>`),t.document.close(),window.setTimeout(()=>{try{t.focus(),t.print()}catch{}},300),!0}var ee="/v2/antrol/search",te="sub=update_v2",ne="/public/antrian-farmasi-v2/list-antrian-v2";async function re(e){try{let t=await fetch(await g()+"/api/queue/lookup?resep_id="+encodeURIComponent(e),{cache:"no-store",credentials:"omit"});if(!t.ok)return null;let n=await t.json();return!n.ok||!n.found||!n.queue?.queue_number?null:{queue_number:n.queue.queue_number,status:n.queue.status??""}}catch{return null}}async function E(e){let t=[e.get("id_resep")||"",e.get("id_resep","nomor_resep")||"",new URLSearchParams(location.search).get("id")??""].filter(n=>n&&n.length>=3);for(let n of t){let r=await re(n);if(r)return r}return null}function O(e){if(e==="DIBATALKAN")return!0;try{let r=(document.querySelector("#isi, .card, .panel, .form-horizontal, form, table")||document.body).querySelectorAll("span, b, strong, td, .label, .badge, h3, h4");for(let i of r){let o=(i.textContent||"").trim();if(/^(batal|dibatalkan|resep batal|sudah dibatalkan)$/i.test(o)&&!i.closest("button, input, a"))return!0}}catch{}return!1}async function oe(e){return fetch(`${ee}?${te}`,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`id=${encodeURIComponent(e)}&taskid=6`,credentials:"include"}).then(t=>(console.log("[MORBIS Ext] antrian terdaftar id="+e,"status",t.status),!0)).catch(t=>(console.warn("[MORBIS Ext] gagal mendaftarkan antrian",t),!1))}async function ie(e,t){try{let n=await fetch(ne,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:"type=check_antrian",cache:"no-store",credentials:"include"});if(!n.ok)return null;let r=await n.json();if(!Array.isArray(r))return null;let i=String(t??"").slice(0,16);return r.find(o=>String(o.ID_PASIEN??"")===String(e)&&(!i||String(o.WAKTU??"").slice(0,16)===i))??r.find(o=>String(o.ID_PASIEN??"")===String(e))??null}catch{return null}}async function I(e,t,n,r){if(!await oe(e))throw new Error("gagal antrol MORBIS");let o=r.get("id_pasien")||new URLSearchParams(location.search).get("norm")||"",a=r.get("waktu_pengajuan"),s=r.namaPasien(),l=n==="racik"?"racikan":"tunggal",d=(async()=>{for(let v=0;v<3;v++){let k=await ie(o,a);if(k)return k;await new Promise(G=>setTimeout(G,200))}return null})(),N=b({event_id:h("enq",e,e+"-"+l)+"-"+Date.now().toString(36),event:"ENQUEUE",resep_id:t,nama_pasien:s,norm:o||void 0,tgl_lahir:r.tglLahir()||void 0,shift:"",jenis:l,counter:"",payload:{idVisit:e,unit:"",waktu:a||""}}),[q,w]=await Promise.all([d,N]);if(!w.ok)throw new Error("gagal terhubung App Antrian");let x=w.queue_number||"";if(!x)throw new Error("nomor antrian belum terbit");return y({nomorResep:t,nama:s,jenis:l,unit:String(q?.NAMA_UNIT??""),tanggal:a?a.slice(0,10):"",code:x,tglLahir:r.tglLahir()}),x}async function P(e,t,n){let r=h("bat",t,e)+"-"+Date.now().toString(36);return(await b({event_id:r,event:"BATAL",queue_number:e,resep_id:t})).ok?{ok:!0,gone:!1}:await E(n)?{ok:!1,gone:!1}:{ok:!0,gone:!0}}function U(e,t,n,r="",i="",o=""){y({nomorResep:e,nama:n.namaPasien(),jenis:r,unit:i,tanggal:o,code:t,tglLahir:n.tglLahir()})}if(window.__extAntrolShift)throw new Error("skip double inject farmasiAntrolShift");window.__extAntrolShift=!0;function ae(){let e=document.querySelector("#nama_pasien")?.value?.trim();if(e)return e.toUpperCase();let t=document.querySelector("#nama")?.value?.trim();if(t)return t.toUpperCase();let n=Array.from(document.querySelectorAll("th, td, label, strong, b, span"));for(let o of n){let a=(o.textContent||"").trim();if(!/^nama\s*pasien$/i.test(a))continue;let s=o.nextElementSibling||o.parentElement?.querySelector("input, select")||o.parentElement?.nextElementSibling,l=(s?.textContent||s?.value||"").trim();if(l)return l.toUpperCase()}let r=/(resep|penjualan|antrian|farmasi|penerimaan|pendaftaran|detail|edit|input|rekap|daftar|shift|cetak|pembayaran|penyerahan|racik|racikan|obat|kasir|pilih|aturan|pakai|dosis|jumlah|satuan|harga|total|biaya|unit|depo|kekuatan|tipe|standar|kronis|klaim|inacbgs|batch|aksi|tambah|selesai|hapus|kembali|simpan)/i,i=Array.from(document.querySelectorAll("h1, h2, h3, .page-title, .card-title"));for(let o of i){if(o.closest('.modal, .modal-header, .modal-body, .dropdown, .dropdown-menu, [role="dialog"]'))continue;let a=(o.textContent||"").trim();if(!(!a||a.length<4||a.length>60||r.test(a)||a.split(/\s+/).filter(Boolean).length<2))return a.toUpperCase()}return""}function se(){let e=document.querySelector("#tgl_lahir")?.value?.trim();if(e)return e;let t=document.querySelectorAll("tr");for(let n of t){let r=n.querySelectorAll("td");for(let i=0;i<r.length-1;i++)if(/^tanggal\s*lahir$/i.test(r[i].textContent?.trim()||"")&&r[i+1]){let o=r[i+1].textContent?.trim();if(o&&o!==":")return o}}return""}var f={get:(e,t)=>(document.querySelector("#"+e)?.value||(t?document.querySelector('input[name="'+t+'"]')?.value:"")||"").trim(),namaPasien:ae,tglLahir:se};function m(e,t){return document.querySelector("#"+e)?.value?.trim()||(t?document.querySelector('input[name="'+t+'"]')?.value?.trim():"")||""}function le(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function u(e,t){let n=document.querySelector("#ext-antrian-bar");if(!n)return;let r=m("nomor_resep","id_resep");if(e==="issued"&&t){let o=le(t);n.innerHTML='<div style="display: flex; flex-direction: column; align-items: flex-start; width: 100%; gap: 6px;"><span style="font-size:18px;font-weight:800;color:#198754;line-height:1.3;">\u2713 Sudah antri \u2014 '+o+'</span><div style="display: flex; gap: 6px;"><button id="ext-antrian-cetak" class="btn" style="margin:0;background:#6c757d;color:#fff;border-color:#6c757d;" title="Cetak ulang kartu tanpa mengantrikan lagi">Cetak Kembali</button><button id="ext-antrian-batal" class="btn" style="margin:0;background:#dc3545;color:#fff;border-color:#dc3545;" title="Hapus antrian dari DB \u2014 resep bisa di-antrikan ulang">Batal antrian</button></div></div>',n.querySelector("#ext-antrian-cetak")?.addEventListener("click",()=>{if(r)try{U(r,t||"",f)}catch{}}),n.querySelector("#ext-antrian-batal")?.addEventListener("click",async()=>{if(!confirm("Batalkan antrian "+t+"? Resep akan keluar dari daftar panggilan."))return;let a=n.querySelector("#ext-antrian-batal");if(a&&(a.disabled=!0,a.textContent="Membatalkan\u2026"),!(await P(t||"",r,f)).ok){alert("[MORBIS Ext] Gagal membatalkan antrian. Coba lagi."),a&&(a.disabled=!1,a.textContent="Batal antrian");return}u("ready")});return}n.innerHTML='<button id="ext-antrian-racik" class="btn" style="margin:2px 6px 2px 0;background:#d97706;color:#fff;border-color:#d97706;" title="Antrikan sebagai obat RACIKAN (nomor R-XX)">Antrikan obat racik</button><button id="ext-antrian-tunggal" class="btn" style="margin:2px 0;background:#2193cf;color:#fff;border-color:#2193cf;" title="Antrikan sebagai obat TUNGGAL (nomor T-XX)">Antrikan obat tunggal</button>';let i=o=>{let a=m("id_visit"),s=m("id_resep","nomor_resep");if(!a||!s){alert("[MORBIS Ext] data resep belum dimuat. Coba lagi.");return}let l=document.querySelector(o==="racik"?"#ext-antrian-racik":"#ext-antrian-tunggal");l&&(l.disabled=!0,l.textContent="Memproses\u2026"),I(a,s,o,f).then(d=>u("issued",d)).catch(d=>alert("[MORBIS Ext] "+(d?.message||"gagal mengantrikan"))).finally(()=>{l&&(l.disabled=!1,l.textContent=o==="racik"?"Antrikan obat racik":"Antrikan obat tunggal")})};n.querySelector("#ext-antrian-racik")?.addEventListener("click",()=>i("racik")),n.querySelector("#ext-antrian-tunggal")?.addEventListener("click",()=>i("tunggal"))}function ce(){let e=()=>{let n=Array.from(document.querySelectorAll('td[valign="top"]')).find(o=>o.querySelector('fieldset#perhatian, fieldset[id="perhatian"]'));if(!n)return null;let r=document.createElement("fieldset");r.id="ext-antrian-fieldset",r.style.cssText="margin-top:6px;",r.innerHTML="<legend>Antrian Farmasi</legend>";let i=document.createElement("div");return i.id="ext-antrian-bar",i.style.cssText="display:flex;flex-wrap:wrap;align-items:center;",r.appendChild(i),n.appendChild(r),i},t=()=>{let r=document.querySelector("#ext-antrian-bar")||e();if(!r)return;let i=o=>{if(!m("id_resep","nomor_resep")){o<10?window.setTimeout(()=>i(o+1),800):u("ready");return}E(f).then(s=>{if(O(s?.status)){r.innerHTML='<span style="color:#b02a37;font-weight:700;">Resep dibatalkan \u2014 antrian tidak tersedia</span>';return}s?u("issued",s.queue_number):o<10?window.setTimeout(()=>i(o+1),800):u("ready")})};i(0)};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t,{once:!0}):t(),window.setTimeout(t,2e3),window.setTimeout(t,5e3)}L(()=>{de(),ce()});function de(){let e=(i,o)=>String(i??"").includes("/v2/antrol/search")&&String(i??"").includes("sub=update_v2")&&String(o??"").includes("taskid=6"),t=XMLHttpRequest.prototype.open,n=XMLHttpRequest.prototype.send;XMLHttpRequest.prototype.open=function(i,o,...a){return this.__extUrl=String(o),t.apply(this,[i,o,...a])},XMLHttpRequest.prototype.send=function(i){if(e(this.__extUrl,i)){console.log("[MORBIS Ext] antrol otomatis diblokir (pakai tombol Antrian & Cetak)");return}return n.apply(this,[i])};let r=window.fetch.bind(window);window.fetch=((i,o)=>{let a=typeof i=="string"?i:i instanceof URL?i.toString():i.url;return e(a,o?.body)?(console.log("[MORBIS Ext] antrol otomatis diblokir (pakai tombol Antrian & Cetak)"),Promise.resolve(new Response(null,{status:200}))):r(i,o)})}})();
